 ...
 *
 * @see the test/basic/create-testOSDLEmbeddedFileSystem-archive.sh script, to
 * generate a test archive for this test.
 *
 */
int main( int argc, char * argv[] )
{

  {
	...
	try
	{
	  ...
	  /*
	   * Note this does not imply that EmbeddedFileSystemManager became the
	   * default filesystem manager: it is still the standard one (luckily for
	   * the log files to remain readable).
	   *
	   */
	  ...
	  /*
	   * If test/basic/create-testOSDLEmbeddedFileSystem-archive.sh has been
	   * run, test-OSDLEmbeddedFileSystem-archive.oar should be listed in the
	   * search path:
	   *
	   */
	  ...
	  /*
	   * This filesystem is not set as the default, thus using it directly here
	   * instead of File::Create.
	   *
	   * Note the result is used as a generic file, regardless of its creator.
	   *
	   */
	  ...
	  /*
	   * Note that it is the embedded filesystem manager (myFSManager) not the
	   * standard (native) that must be used, as platform-independent paths are
	   * expected here:
	   *
	   */
	   ...
	   * the chooseBasicSettings call (due to the default parameter), thus the
	   * archives with this extension in default paths have already been
	   * mounted:
	   *
	   */
