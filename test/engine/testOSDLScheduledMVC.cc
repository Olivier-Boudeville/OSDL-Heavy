 ...
 *
 *  - regularly the scheduler, thanks to its input, simulation and rendering
 * ticks, takes in charge the corresponding tasks with the specified frequencies
 *
 *  - whenever an input tick occurs, the input devices are read and the
 * controller state is updated according to the keyboard arrows having been
 * pressed or the first joystick moved
 *
 *  - whenever a simulation tick occurs, the model gets informations from it
 * controller and updates its own state
 *
 *  - whenever a rendering tick occurs, the basic renderer being used just
 * request each view to render itself.
 *
 * @note This test is said to be schedule-driven since it relies on a scheduler
 * to organize in turn all the activation of the MVC actors. A simpler yet less
 * versatile way of handling the MVC framework is to behave as an event-driven
 * MVC framework, where the clock of the system is given by incoming new input
 * events, which trigger in turn the controllers, the models and then the views.
 *
...
public:
  ...
  /**
   * Called by the scheduler when a new rendering tick occurs, makes the
   * views display themselves.
   *
   */
...
/**
 * Testing OSDL MVC integration with input layer, when using a scheduler (not
 * event-driven).
 *
...
int main( int argc, char * argv[] )
{

  {
	...
	try
	{
	  ...
	  /*
	   * Create and link MVC instances (new operator and pointers should be used
	   ...
	   * test for robustness against misuse).
	   *
	   */
	  ...
	  /*
	   * Logs such as "EventListener subscribed to source '^'" are normal since
	   * views display as '^'.
	   *
	   */
	   ...
	   *
	   * Warning: this renderer must be specially allocated thanks to a new(),
	   * and not simply as an automatic variable, since the renderer will
	   * automatically register itself to the scheduler which will take
	   * ownership of it, and will deallocate it upon its own destruction.
	   *
	   * Therefore it must not be an automatic variable whose life cycle would
