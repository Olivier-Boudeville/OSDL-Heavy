 ...
 * more information about the MP3 playback, and a different load request, as no
 * header can be used here (th goal being to find the values to put in that
 * header).
 *
 */
...
bool decodeNextMP3Frame()
{
	...
	if ( readPointer > endOfSafeRead )
	{
		 ...
		 *
		 * - for the destination: let alpha be the number of remaining bytes if
		 * read buffer. alpha = doubleBuffer + 2*bufferSize - readPointer
		 * Destination is ( doubleBuffer + frameSizeUpperBound - alpha ) so that
		 * these remaining bytes will be moved just before the next ones to be
		 * played in first buffer.
		 *
		 ...
		 * doubleBuffer + 2 * bufferSize - readPointer i.e. Size = sizeOffset -
		 * readPointer
		 *
