...
namespace OSDL
{
	...
	namespace Utils
	{
		 ...
		 *
		 * @throw OSDL::Exception if the separator cannot be described as a
		 * Latin1 character (ex: if ir involves multiple characters).
		 *
		 ...
		 *
		 * This is the program base directory, i.e. the directory where it was
		 * run from, which is probably the installation directory, and may or
		 * may not be the process's current working directory.
		 *
		 * @note This base directory can be included in the search path of any
		 * embedded filesystem.
		 *
		 ...
		 * users, either.
		 *
		 * @note This user directory can be included in the write path of any
		 * embedded filesystem, also be put near the beginning of the search
		 * path.
		 *
		 * @see OSDL::EmbeddedFilesystem.
		 *
		...
		/**
		 * A DataStream is a way of writing and/or reading for an opaque data
		 * storage, which can be actually a file, a buffer in memory, or
		 * anything else.
		 *
		...
		/**
		 * Returns a DataStream corresponding to specific file object, which can
		 * be of any sort (ex: standard file, embedded file, etc.).
		 *
		 * @param sourceFile the file from which the DataStream will be created.
		 *
		 * @note Ownership of the specified is taken, thus the user should not
