...
namespace OSDL
{
	...
	/**
	 * Encapsulates embedded files, i.e. files in a virtual filesystem stored in
	 * an actual standard file, as provided by the PhysicsFS back-end.
	 *
	 * @see http://icculus.org/physfs/
	 *
	 * @note All files are managed in binary mode only. Reads and writes will be
	 * cyphered, depending on the filesystem manager setting.
	 *
	 * Actual files should be created and opened with respectively the
	 * File::Create and File::Open factories, that allow the user program to be
	 * cross-platform by hiding each filesystem-related per-platform
	 * specificity.
	 *
	 */
	class OSDL_DLL EmbeddedFile : public Ceylan::System::File
	{
		...
		public:
			...
			/**
			 * Destroys the embedded file reference object, does not remove the
			 * file itself.
			 *
			 ...
			 * physical media, cannot store the data for some reason. In such a
			 * case, the file handle stays open.
			 *
			 ...
			 *
			 * @throw FileReadLockingFailed if the operation failed or if the
			 ...
			 *
			 * @throw FileReadUnlockingFailed if the operation failed or if the
			 * file lock feature is not available.
			 *
			 */
			 ...
			 *
			 * @throw FileWriteLockingFailed if the operation failed or if the
			 * file lock feature is not available.
			 *
			 */
			 ...
			 *
			 * @throw FileWriteUnlockingFailed if the operation failed or if the
			 * file lock feature is not available.
			 *
			 */
			 ...
			 * locked, otherwise returns false, i.e. if the file is locked or if
			 * the lock feature is not available.
			 *
			 */
			 ...
			 *
			 * @param buffer the buffer where to store read bytes. Its size must
			 * be at least maxLength bytes.
			 *
			 * @param maxLength the maximum number of bytes that should be read.
			 *
			 * @return The number of bytes actually read, which should be
			 * maxLength or lower.
			 *
			 * @throw InputStream::ReadFailedException if a read error
			 * occurred. Note that this is not a child class of FileException,
			 * as it comes from an inherited interface.
			 *
			...
			/**
			 * Writes up to maxLength bytes from the specified buffer to this
			 * file.
			 *
			 * @param buffer the buffer where to find bytes that must be written
			 * to this file. Its size must be at least maxLength bytes.
			 *
			 * @return The number of bytes actually written, which should be
			 * equal to maxLength.
			 *
			 ...
			 *
			 * @param targetPosition this position corresponds to the number of
			 * bytes from start of file to seek to.
			 *
			 * @throw FileException if the operation failed.
			 *
			 */
			 ...
			 * supported on this platform, and FileDelegatingException if the
			 * corresponding filesystem manager could not be used as expected.
			 *
			 */
			...
			/**
			 * Sends the file content to the <b>fd</b> PhysicsFS handle stream.
			 *
			 * @throw EmbeddedFileException if the operation failed or if the
			 * PhysicsFS handle feature is not available.
			 *
			 */
			 ...
			 *
			 * @note Public, so that very specific tools can call it (ex:
			 * cypherOSDLFile).
			 *
			 */
			 ...
			 *
			 * @return the StreamID or -1 if nothing appropriate can be returned
			 ...
			 * Helper section.
			 *
			 * Factories still have to be public, to allow to create on specific
			 * cases (ex: process redirection) specifically embedded files, not
			 * only files.
			 *
			 */
			 ...
			 *
			 * @param createFlag the flag describing the creation mode. Only
			 * binary access is performed.
			 *
			 ...
			 *
			 * @note This EmbeddedFile factory is only a helper method. Ceylan
			 * users should only use File::Create instead.
			 *
			...
			/**
			 * Returns an EmbeddedFile reference on specified already-existing
			 * file, in platform-independent notation, which will be opened with
			 * specified settings.
			 *
			 ...
			 * @see OpeningFlag
			 *
			 * @note This EmbeddedFile factory is only a helper method. OSDL
			 * users should only use File::Open instead.
			 *
			 * @throw FileException, including FileOpeningFailed if the
			 * operation failed or is not supported on this platform.
			 *
			 * Note that, depending on the archive type, opening the same file
			 * more than once may or may not work (read content apparently could
			 * be scrambled when opened twice).
			 *
			 */
		...
		protected:
			 ...
			 * @see OpeningFlag, PermissionFlag
			 *
			 * @note If not specifically set, the file is open in text mode: one
			 * should not forget to add the Binary flag.
			 *
			 * The mistake can be detected when basic read() returns less than
			 * the requested size, or when readExactLength() never terminates.
			 *
			 * @note This constructor should not be called directly, the File
			 * factories (File::Create and File::Open) should be used instead,
			 * as they allow to write code really independant from the running
			 * platform, not having to choose between the per-platform
			 * constructors.
			 *
