...
namespace OSDL
{
     ...
     * seamlessly performed on an abstract filesystem, stored in the set
     * of opaque files and archives.
     *
    	...
    	public Ceylan::System::FileSystemManager, public Ceylan::Object
	{
		...
		public:
             ...
             *
             * When you mount an archive, it is added to a virtual file
             * system.
             * All files in all of the archives are referenced into a single
             * virtual hierarchical file tree.
             * Two archives mounted at the same place (or an archive with 
             * files overlapping another mountpoint) may have overlapping 
             * files: in such a case, the file earliest in the search path 
             * is selected, and the other files are inaccessible to the
             * application.
             * This allows archives to be used to override previous revisions;
             * you can use the mounting mechanism to place archives at a
             * specific point in the file tree and prevent overlap; this
             * is useful for downloadable mods that might trample over
             * application data or each other, for example.
             *
             * The mountpoint does not need to exist prior to mounting, 
             * which is different than those, familiar with the Unix concept 
             * of "mounting", may not expect.
             ...
             *
			 * @param newActualFilesystemElement the directory or archive 
             * to add to the search path, in platform-dependent notation.
             *
			 * @param mountPointInVirtualTree location in the virtual tree
             * this element will be mounted on, in platform-independent
             * notation. An empty string is equivalent to "/".
             *
			 * @param append if true append to search path, otherwise
             * prepend.
             *
			 * @throw EmbeddedFileSystemManagerException if the operation
             ...
             * search path.
             *
			 * This must be a (case-sensitive) match to a directory or
             * archive already in the search path, specified in
             * platform-dependent notation.
             *
             * @param actualFilesystemElement the directory or archive to 
             * remove from search path.
             *
             * @note This call will fail (and fail to remove the element)
             * if at least a file remains open in it.
             *
			 * @throw EmbeddedFileSystemManagerException if the operation
             * failed (bogus archive, directory missing, etc.).
			 *
			 ...
			 *
             * @note The specified entry will searched through the full 
             * search path. Entries that are symlinks are ignored, unless
             * the symlink status has been set to true.
             *
			...
			/**
			 * Returns the directory separator in the embedded filesystem, 
             * a Latin-1 character.
			 *
			 ...
			 *
			 * @note embeddedFile helper factory, has to be public but the
             ...
             * opening a file.
             *
             * So, if you look for "maps/level1.map", and C:\\mygame is in 
             * your search path and C:\\mygame\\maps\\level1.map exists, then
             ...
             * there, even though that directory is not necessarily contained 
             * in a real archive.
             *
			 ...
			 * test failed with no answer) or is not supported on this 
			 * platform, or EmbeddedFileSystemManagerException is the file
             * was not found.
			 *
			 */
             ...
             * the search path is a symbolic link ( directory entry.
             *
             ...
             * platform-independent notation in relation to the
             * write directory.
			 *
             ...
             * filename  to remove is "downloads/maps/level1.map", then the 
             * file "C:\mygame\writedir\downloads\maps\level1.map" is removed
             * from the physical filesystem, if it exists and the operating
             * system permits the deletion.
             *
             * @note On Unix systems, deleting a file may be successful, but
             * the actual file will not be removed until all processes that 
             * have an open filehandle to it (including your program) close
             * their handles.
             *
             * Chances are, the bits that make up the file still exist, they
             * are just made available to be written over at a later point.
             ...
             *
             * @note The file size may not be determined (ex: since the 
             * archive is "streamed").
             * Also note that if another process/thread is writing to this 
             * file at the same time, then the information this function
             * supplies could be incorrect before you get it.
             * Use with caution, or better yet, do not use at all.
             *
			 * @throw FileSizeRequestFailed if the operation failed (ex:
			 * file not found, size could not be determined, etc.) or is 
			 ...
			 * searched, in platform-independent notation.
			 *
             * @return The modification time is returned as a number of 
			 ...
			 * failed, if there is no reasonable way to obtain this information
             * for a particular archiver, or if it is not supported on this
             * platform.
			 *
			 */
             ...
             * By default, a file will be checked to know whether it is
             * really a symlink during open calls and, the operation will
             * fail if it is.
             * Otherwise, the link could take you outside the write and search
             * paths, and compromise security.
             *
             * If you want to take that risk, call this function with 
             ...
             *
             * Symlinks are only explicitly checked when dealing with 
             * filenames in platform-independent notation.
			 ...
			 *
			 * @note embeddedDirectory helper factory, has to be public 
			...
			/**
			 * Determines if the first occurence of <b>directoryPath</b> in 
             * the search path is really a directory entry.
             *
			 * @param directoryPath the directory path to look-up, in
             * platform-independent notation.
			 *
