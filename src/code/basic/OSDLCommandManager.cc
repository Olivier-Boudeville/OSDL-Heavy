...
void CommandManager::playMusic( Audio::Music & music )
{
	 ...
	 * whether the playback should start from first buffer (if 1) or from second
	 * (if 0).
	 *
	 * The second element is a pointer to the cache-aligned sound buffer, third
	 * is composed of the full size of the encoded buffer (16 first bits), then
	 * the delta value (the MP3 frame size upper bound, 16 last bits).
	 *
	 */
...
void CommandManager::stopMusic()
{
	...
	/*
	 * manageNoMoreCurrent and al will be called when the ARM7 notifies the end
	 * of playback.
	 *
	 */
	...
	FIFOCommandID commandID, Ceylan::System::FIFOElement firstElement )
{
	...
	switch( commandID )
	{
		...
		/*
		 * @note Filling buffers must be done in the main loop, not in this IRQ
		 * handler, thus just requesting the filling, not performing it:
		 *
		 */
