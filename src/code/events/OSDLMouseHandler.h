...
namespace OSDL
{
	...
	namespace Events
	{
		 ...
		 *
		 * Manages the mouse position, its up to three buttons, and a possible
		 * mouse wheel (up/down).
		 *
		...
		class OSDL_DLL MouseHandler : public InputDeviceHandler
		{
			...
			/*
			 * The events module has to trigger the mouse event callbacks of
			 * this mouse handler.
			 *
			 */
			...
			public:
				 ...
				 *
				 * @param useClassicalMice tells whether detected mice should be
				 * managed as complex mice (powerful interface, but it requires
				 * the controller to take in charge this complexity) or
				 * classical mice (for usual needs, comes with preset behaviours
				 * which ease controller work).
				 *
				 * @throw InputDeviceHandlerException if the initialization of
				 * the mouse subsystem failed.
				 *
				 */
				...
				/**
				 * Links the default mouse to specified controller, so that any
				 * further mouse-related event will be sent to this controller.
				 *
				 * Removes automatically any link previously defined between the
				 * mouse and any other controller.
				 *
				 ...
				 * mouse and any other controller.
				 *
				 * @param index the index of the mouse to link, in case there
				 * are other mice than the main default one.
				 *
				 * @param controller the OSDL controller which will be notified
				 * of mouse changes.
				 *
				...
				/**
				 * Returns the number of available mice, i.e. the mice currently
		...
		protected:
				...
				/**
				 * Called whenever a mouse is moved, so that its controller, if
				 * any, is notified.
				 *
				 * @note Expected to be triggered by the EventsModule.
				 *
				...
				/**
				 * Called whenever a mouse button was pressed, so that its
				...
				/**
				 * Called whenever a mouse button was released, so that its
				 * controller, if any, is notified.
				 *
				 * @note Expected to be triggered by the EventsModule.
				 *
				...
				/**
				 * Checks that the mouse at the specified index exists and is
				 * opened.
				 *
				...
				/**
				 * Tells whether complex mice (if false) or classical mice (if
				 * true) should be created.
				 *
				 */
