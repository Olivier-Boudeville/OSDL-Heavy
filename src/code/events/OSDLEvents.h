...
namespace OSDL
{
	...
	namespace Events
	{
		...
		/**
		 * A tick is the most precise time unit that can be measured by the
		 * system.
		 *
		...
		/**
		 * Simulation tick is the most fine unit of time between two simulation
		 * steps.
		 *
		 * @note Their duration is an integer multiple of the duration of the
		 * engine ticks.
		 *
		 */
		...
		/**
		 * Rendering tick is the most fine unit of time between two rendering
		 * steps.
		 *
		 * @note Their duration is an integer multiple of the duration of the
		 * engine ticks.
		...
		/**
		 * Rendering ticks is the most fine unit of time between two input
		 * pollings.
		 *
		 * @note Their duration is an integer multiple of the duration of the
		 * engine ticks.
		 ...
		 *
		 * Period is a specialization of engine ticks in the case of periodic
		 * events.
		 *
		 */
		...
		/**
		 * Handles the asynchronous event-based interaction model, general input
		 * handling, and basic time management, including the logic and
		 ...
		 * activation of the scheduled objects, be it periodic or specifically
		 * programmed.
		 *
		 ...
		 * against this process, which otherwise would be deemed too CPU-hungry.
		 *
		 * The user can supply a specific idle callback, if wanted.
		 *
		 * Under all circumstances, the callback function should last for very
		 * small durations, compared to the frequencies requested (better
		 * multiple idle calls than too long too few), otherwise the scheduling
		 * may constantly fail.
		 *
		 * Event module notes:
		 *
		 * The event modules comes with no joystick handler registered, but with
		 * a keyboard handler.
		 *
		 * Scheduler notes:
		 *
		 * @note Even though logic and rendering are not as tightly linked as
		 * they often are, if the sum of their processing needs exceeds what the
		 * hosting computer can give, they will have to overcome this resource
		 * limitation.
		 *
		 * The logic is deemed more important than rendering, therefore jerked
		 * graphics will be preferred by the scheduler to frame-rate dependent
		 * logic.
		 *
		 ...
		 *
		 * - a simulation tick lasts for 10 engine ticks (logic frequency: 100
		 * Hz)
		 *
		 * - a rendering tick lasts for 25 engine ticks (rendering frequency,
		 * also known as framerate: 40 Hz, i.e. 40 frames per second), if there
		 * is enough processing power
		 *
		...
		class OSDL_DLL EventsModule : public Ceylan::Module
		{
			...
			public:
				 ...
				 *
				 * Will not work with the SDL back-end unless a window is opened
				 * thanks to VideoModule::setMode: otherwise, no event would be
				 * generated.
				 *
				 * @param displayWaitingMessage the default waiting message will
				 * be displayed iff true.
				 *
				 ...
				 * parameter <b>seconds</b>, <b>micros</b> should be less than
				 * one second, i.e. should be in the range 0 to 10E6 - 1.
				 *
				 ...
				 *
				 * If a scheduler is wanted but does not already exist, it will
				 * be created with default settings.
				 *
				 * This scheduler will be in charge of all the active objects
				 * that will be registered subsequently.
				 *
				 * @param on tells whether a scheduler is wanted.
				 *
				 * @see Scheduler::GetScheduler method to further customize the
				 * scheduler.
				 *
				 */
				...
				/**
				 * Sets the idle function, which is called whenever the current
				 * scheduling system, either the main event loop or the
				 * scheduler, detects a period of idle activity.
				 *
				 * The callback can be useful when dealing with animations for
				 * example, or to execute background activities (such as
				 * counting the frame rate), or to let the process yield some
				 * CPU time so that the OS does not consider it as too
				 * CPU-hungry, and does not take counter-measures against it.
				 *
				 * @note Ensure that the maximum time spent in the callback is
				 * negligible compared to the period corresponding to the
				 * maximum frequency being set, so that it does not result in a
				 * too heavy load which would make the scheduler fail
				 * constantly.
				 *
				 * @param idleCallback the idle callback, which can be null (0)
				 * to specify no idle callback is wanted.
				 *
				 ...
				 * main loop enforcing its target frequency. If this value is
				...
				/**
				 * Sets the actual target frequency that the basic event loop
				 * will try to run at.
				 *
				 * @note Will not take effect until the next call to the basic
				 * event loop.
				 *
				 */
				 ...
				 *
				 * If a scheduler is being used, will start it, and stop it on
				 * exit.
				 *
				 ...
				 *
				 * @note This method is useful for input handlers to send too
				 * quit notifications, otherwise the only way of quitting would
				 * be provided by the window manager, if any.
				 *
				 */
				 ...
				 *
				 * @note If a keyboard handler was already registered, it is
				 * unregistered and deallocated first.
				 *
				 */
				 ...
				 *
				 * @note If a joystick handler was already registered, it is
				 * unregistered and deallocated first.
				 *
				 */
				 ...
				 *
				 * @note If a mouse handler was already registered, it is
				 * unregistered and deallocated first.
				 *
				 */
				 ...
				 *
				 * Reads all pending low-level events and dispatch them to the
				 * appropriate manager.
				 *
				 ...
				 * no need to explicitly retrieve the common module, then events
				 * module before knowing the result.
				 *
				...
				/**
				 * Tries to guess an upper-bound for the average duration of the
				 * execution of the specified callback.
				 *
				...
				/*
				 * Basic event types, used to discriminate in the generic event
				 * structure which is the actual type.
				 *
				 */
				...
				/**
				 * Identifies the event corresponding to application gaining or
				 * loosing focus(es).
				 *
				 */
				...
				/**
				 * Identifies the event corresponding to a joystick trackball
				 * motion.
				 *
				 */
				...
				/**
				 * Identifies the event corresponding to a change of joystick
				 * hat position.
				 *
				 */
				...
				/**
				 * Identifies the event corresponding to a joystick button being
				 * pressed.
				 *
				 */
				...
				/**
				 * Identifies the event corresponding to the user requesting to
				 * quit.
				 *
				 */
		...
		protected:
				 ...
				 *
				 * @note Made to be overriden, basically just writes the event
				...
				/**
				 * Called whenever at least one of the three focuses (mouse,
				 * keyboard or application visibility) changed.
				 *
				 * @note Call specialized methods such as onMouseFocusGained to
				 * handle each case.
				 *
				 */
				...
				/**
				 * Called whenever a system-specific window manager event is
				 * received.
				 *
				 * These unhandled events are received from the window manager,
				 * and can be used, for example, to implement cut-and-paste in
				 * your application.
				 *
				 */
				 ...
				 *
				 * Only applications that have been specified as resizable in
				 * the setMode call can receive that event.
				 *
				...
				/**
				 * Called whenever the screen has been modified outside of the
				 * application, usually by the window manager, and needs to be
				 * redrawn.
				 *
				 */
				...
				/**
				 * Called whenever an unknown event type occurs during the input
				 * update phase.
				 *
				 */
				 ...
				 *
				 * @note By default, a basic event loop is used, instead of a
				 * full-blown scheduler.
				 *
				 */
				bool _useScheduler ;
				 ...
				 *
				 * Should there be time left for an iteration, then the event
				 * loop will call the idle callback multiple times, until the
				 * next iteration is to occur.
				 *
				 * If no idle callback is registered, then for each iteration
				 * with time left, the event loop will call the basic sleep
				 * mecanism, as provided by the operating system.
				 *
				...
				/**
				 * Count the number of idle calls made during the current event
				 * loop.
				 *
				 */
				...
				/**
				 * The idle callback, if any, to be called by the basic event
				 * loop.
				 *
				 */
				...
				/**
				 * The data, if any, to provide to the idle callback, should the
				 * basic event loop be used.
				 *
				 */
				...
				/**
				 * Header of messages sent by default implementations (ex:
				 * onMouseFocusGained).
				 *
				 */
		...
		private:
				 ...
				 *
				 * @param eventsFlag flag describing which input device should
				 * be enabled (keyboard, mouse, joystick, etc.)
				 *
				 * @throw EventsException if the initialization of an input
				 * subsystem failed.
				 *
				 * @note Some input device handlers (ex: mouse or joystick) may
				 * be created even though not specifically requested, because
				 * some events may have to be managed anyway (ex: keyboard or
				 * mouse focus lost, etc.).
				 *
				 */
