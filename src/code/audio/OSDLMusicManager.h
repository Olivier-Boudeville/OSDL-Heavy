	...
	namespace Audio 
	{
		...
		class OSDL_DLL MusicPlaybackSetting : public Ceylan::TextDisplayable
		{
			...
			public:
				 ...
				 *
				 * @param count the number of times this music should be 
				 * played (-1 means infinite).
				 *
				 */
				 ...
				 * settings.
				 *
				 * Does not return whether this playback should be kept, 
				 * as anyway it should be removed once the playback is over,
				 * so that no music still being played back is removed if its
		...
		/**
		 * A music manager has for role to drive the playback of musics, 
		 * based on a playlist.
		 *
		 * Although multiple managers can be used simultaneously, usely there
		 * is only one of them, statically registered by the audio module.
		 *
		 * @see Music
		 *
		 */
		class OSDL_DLL MusicManager: public Ceylan::Object
		{
			...
			public:
				 ...
				 * other musics in the playlist (if any) will have been
				 * played in turn.
				 *
				 */
