...
namespace OSDL
{
	...
	namespace Audio
	{
		 ...
		 *
		 * Methods for playing sounds are non-blocking: they trigger the playing
		 ...
		 *
		 * In the embedded case (Nintendo DS platform), sounds can be either RAW
		 * with OSDL header, or MP3.
		 *
		 * Sounds, as opposed to musics, are read once for all, as a whole, in
		 * memory, not streamed chunk after chunk from the media.
		 *
		 */
			...
			public Ceylan::LoadableWithContent<LowLevelSound>
		{
			...
			public:
				...
				*/
				 ...
				 *
				 * @throw SoundException if the operation failed, including if
				 * not supported or if no previously loaded sound is available.
				 *
				 */
				...
				/**
				 * Plays this sound instance at once on any appropriate channel
				 * number.
				 *
				 ...
				 * not supported or if no free channel is available.
				 *
				...
				/**
				 * Plays this sound instance at once on any appropriate channel
				 * number, and returns that channel number.
				 *
				 * @param playCount the number of times this sound should be
				 * played, unless stopped by halt, fade out, expiration time or
				 * audio module stop. -1 means forever. Otherwise it must be
				 ...
				 * channel.
				 *
				 * @param mixingChannelNumber number of the mixing channel this
				 * sound should be played on.
				 *
				 * @param playCount the number of times this sound should be
				 * played, unless stopped by halt, fade out, expiration time or
				 * audio module stop. -1 means forever. Otherwise it must be
				 ...
				 * sound will be played, in milliseconds. It may stop earlier
				 * if: (sound duration).playCount < maxDuration
				 *
				 * @param playCount the number of times this sound should be
				 * played, unless stopped by halt, fade out, expiration time or
				 * audio module stop. -1 means forever. Otherwise it must be
				...
				/**
				 * Plays this sound instance at once on any appropriate channel
				 * number, beginning with a fade-in effect.
				 *
				 ...
				 * not supported or if no free channel is available.
				 *
				 * @see playWithFadeInReturnChannel if the chosen channel needs
				 * to be known.
				 *
				 */
				...
				/**
				 * Plays this sound instance at once on specified channel
				 * number, beginning with a fade-in effect.
				 *
				 * @param fadeInMaxDuration duration in milliseconds during
				 * which the fade-in effect should take to go from silence to
