...
namespace OSDL
{
	...
	namespace Audio
	{
		...
		class OSDL_DLL AudioModule : public Ceylan::Module
		{
			...
			/**
			 * This friend function is intended to be the usual means of getting
			 * a reference to the audio module, which must already exist. It is
			 * called in contexts where this module has to exist.
			 *
			 * If not, a fatal error will be triggered (not an exception to
			 * avoid handling it in all user methods).
			 *
			 * @note This function is mainly useful for the OSDL internals, for
			 * example for channel callbacks.
			 *
			...
			public:
				 ...
				 * the encoding of the final samples sent to the audio card (ex:
				 * 16-bit little endian unsigned samples, i.e.
				 * LittleUint16SampleFormat).
				 *
				 * @param outputChannel the channel output format, i.e.  the
				 * number and role of the output streams to target (ex: stereo).
				 *
				 ...
				 *
				 * @throw AudioException if the setting of the new mode failed,
				 * including if the operation is not supported.
				 *
				 */
				 ...
				 * output channels obtained, usually one, for mono, or two, for
				 * stereo (note that is not the channel format).
				 *
				 ...
				 * left and right channel output channels.
				 *
				 * @note This effect will only work on stereo audio output. On
				 * mono audio device, nothing will be done.
				 *
				 ...
				 *
				 * @note AudioChannel::onPlaybackFinished will be automatically
				 * called on halted channels.
				 *
				 * @throw AudioException if the operation failed or is not
				 * supported.
				 *
				...
				/**
				 * Makes all mixing channels stop after the specified duration
				 * is elapsed.
				 *
				 * @param expireDuration the duration, in milliseconds, until
				 * the channels are stopped.
				 *
				 * @return the number of channels set to expire, whether or not
				 * they are active.
				 *
				 * @throw AudioException if the operation failed, including if
				 * not supported.
				...
				/**
				 * Returns the name of the audio driver being currently used
				 * (example: 'dsp').
				 *
				 * @see GetDriverName
				 *
				 * @throw AudioException if the operation failed, including if
				 * the audio driver is not initialized or used by another
				 * application.
				 *
				 */
				 ...
				 * directory names that should contain audio files.
				 *
				 */
				 ...
				 *
				 * Automatically gathers the list of directories specified as
				 * the value of the environment variable named as specified in
				...
				/**
				 * Returns a summary about the possible use of audio-related
				 ...
				 * no need to explicitly retrieve the common module, then audio
...
#pragma warning( pop )
				 ...
				 * channel format.
				 *
				 * @param format the channel format whose number of channels is
				 * to be determined.
				 *
				 ...
				 *
				 * @param format the sample format whose size in bytes is to be
				 * determined.
				 *
			...
			private:
				 ...
				 *
				 * @throw AudioException if the audio subsystem initialization
				 * failed.
				 *
				 */
				AudioModule() ;
		 ...
		 * reference to the audio module, which must already exist. If not, an
