...
namespace OSDL
{
	...
	namespace Rendering 
	{
		 ...
		 * 
		 * Its rendering is mainly triggered by the root renderer, if it exists and if this audio
		 * renderer has been registered to this root renderer.
		 *
		 * Specialized audio renderers can take in charge for example sound propagation,
		 * multi-channel output and cameras.
		 *
		 * @see Ceylan::View
		 *
		 */	
		class AudioRenderer : public Rendering::Renderer
		{
			...
			public:
				 ...
				 *
				 * @param registerToRootRenderer iff true, this audio renderer will 
				 * register itself automatically to the root renderer, which must implement the
				 * MultimediaRenderer interface so that this audio renderer can be plugged.
				 *
				 * @throw RenderingException if registerToRootRenderer is true and no root
				 * renderer is available, or if the root renderer is not able to register audio
				...
				/**
				 * Returns whether this audio renderer has an internal camera available.
				 *
				 */
				 ...
				 *
				 * @note The audio renderer takes ownership of the provided camera, and will
			...
			private:
				 ...
				 * 
				 */			 
				AudioRenderer & operator = ( const AudioRenderer & source ) throw() ;
