...
BoundingBox2D::BoundingBox2D( Locatable2D & father, const Bipoint & center) throw() :
	Locatable2D( father )
{
	setCenter( center ) ;
}
...
const string BoundingBox2D::toString( Ceylan::VerbosityLevels level ) const throw()
{	
...
BoundingBox2D & BoundingBox2D::CheckIs2D( BoundingBox & box ) throw( BoundingBoxException )
{

	BoundingBox2D * box2D = dynamic_cast<BoundingBox2D *>( & box ) ;
	
	if ( box2D == 0 )
		throw BoundingBoxException( "BoundingBox2D::CheckIs2D : specified box ("
