...
namespace OSDL
{
	...
	namespace Rendering 
	{
		...
		/// Exception to be thrown when rendering encounters an abnormal situation.
		class RenderingException : public OSDL::Exception
		{
			public:

				explicit RenderingException( const std::string & reason ) throw() ;
		...
		/**
		 * This basic renderer manages the rendering of all registered views. Note that renderers
		 * and views are not dedicated to the graphical output, since they have to take in charge
		 * all multimedia fields, including sounds and, possibly, other media. 
		 *
		 * The privilegied way of organizing the rendering is to have renderers taking care of all 
		 * aspects of the rendering (video, but also audio, etc.). Renderers are not necessarily
		 * singletons, but having only one centralized instance for all views might be simpler.
		 * In this case it is called a root renderer.
		 * A root renderer may delegate part of its task to specialized sub-renderers.
		 *
		 * More precisely, the scheduler will call only one of them, the root renderer, which may
		 * delegate some of its charges to any number of specialized renderers, for example
		 * if multiple viewports are used in the same window, or for the sound.
		 *
		 * A renderer knows all views (as defined by the MVC framework) it is in charge of, but 
		 * only them (no model for example).
		 *
		 * However no data structure is provided with this generic renderer, since it may vary a 
		 * lot (from simple list to BSP trees, etc.)
		 *
		 *
		...
		class Renderer : public Ceylan::Object
		{
			...
			public:
				 ...
				 *
				 * @param registerToScheduler iff true, then a scheduler is supposed to exist 
				 * already, and this renderer is automatically registered to the scheduler so
				 * that it gets called at each rendering tick.
				 *
				 * @throw RenderingException if registerToScheduler is true and no scheduler is
				 * available.
				 *
				 */
				explicit Renderer( bool registerToScheduler = true ) throw( RenderingException ) ;
	         	...
	         	*
	         	* @note The returned value is a reference and not a pointer, to avoid any abnormal
			 	* deallocation by its users, that should never deallocate the root renderer.
	         	*
				* @throw RenderingException if there is no root renderer already available.
				*
				* @note There is no 'GetRootRenderer' method that would create a root renderer if
				* none was already set, since various renderers could be chosen as root.
				*
	         	*/
			...
			protected:
				...
				/**
				 * Records all the currently registered views that shall be managed specifically
				 * by this renderer.
				 *
				 *
				 */
			...
			private:
				...
				/**
				 * The internal single instance of renderer, at the top of the renderer
				 * hierarchy, if any.
				 *
				 */
