...
namespace OSDL
{
	...
	namespace Rendering
	{
		 ...
		 *
		 * The privileged way of organizing the rendering is to have renderers
		 * taking care of all multimedia aspects (video, but also audio, etc.).
		 *
		 ...
		 * sub-renderers.
		 *
		 * More precisely, the scheduler will call only one of them, the root
		 * renderer, which may delegate a part of its charge to any number of
		 * specialized renderers, for example if multiple viewports are used in
		 * the same window, or for the sound, or to take advantage of multicore
		 * processors.
		 *
		 * A renderer knows all the views (as defined by the MVC framework) it
		 * is in charge of, but only them (not their model, for instance).
		 *
		 * However no data structure is provided with this generic renderer,
		 * since it may vary a lot depending on the need, from simple list to
		 * BSP trees, etc.
		 *
		...
		class OSDL_DLL Renderer : public Ceylan::Object
		{
			...
			public:
				 ...
				 *
				 * @param registerToScheduler iff true, then a scheduler is
				 * supposed to exist already, and this renderer is automatically
				 * registered to this scheduler so that it gets called at each
				 * rendering tick.
				 *
				 * @throw RenderingException if registerToScheduler is true and
				 ...
				 *
				 * @param currentRenderingTick the rendering tick corresponding
				 * to this render step.
				 *
				...
				/**
				 * Allows the renderer to be aware that a rendering step had to
				 * be skipped.
				 *
				 * It may be a chance for it to trigger counter-measures, such
				 * as decreasing the level of detail in order not to slow down
				 * the whole process, or simply notifying the views of the skip.
				 *
				 * @param skippedRenderingTick the rendering tick that had to be
				 * skipped.
				 *
				 */
			   ...
			   /**
				* Returns the one and only one root renderer instance that may
				* be already available.
				*
				* @note The returned value is a reference and not a pointer, to
				* help avoiding any abnormal deallocation by its users, that
				* should never deallocate the root renderer.
				*
				* @throw RenderingException if there is no root renderer already
				* available.
				*
				* @note There is no 'GetRootRenderer' method that would create a
				* root renderer if none was already set, since various renderers
				* could be chosen as the root one.
				*
				*/
			...
			private:
				...
				/**
				 * The internal single instance of renderer, at the top of the
				 * renderer hierarchy, if any.
				 *
				 */
