...
namespace OSDL
{
	...
	namespace Engine 
	{
		...
		/**
		 * Active objects are dynamic objects, i.e. which needs some 
		 * CPU time, since they live. 
		 * They are therefore taken into account by our scheduler, which 
		 * applies simple heuristics so that each active object is granted 
		 * with the processing power it requested, on a best-effort basis,
		 * according to an appropriate timing.
		 *
		 * These objects can be activated on a regular basis, a period 
		 * being specified for them, or on specific simulation steps. 
		 * In this case, a list of simulation steps is to be specified,
		 * absolutely or relatively to the current simulation step. 
		 * An object can be activated on a periodic basis, as a programmed
		 * object, or both.
		 *
		 ...
		 * can be corrected in time (n+1), whereas the second approach 
		 * demands all simulation steps to be processed and therefore would 
		 * not allow the scheduler in the hurry to skip any step. 
		 * However, not all behaviours could follow the first approach. 
		 *
		 ...
		 * object with periodic scheduling. 
		 ...
		 * The recommended first approach would be to compute the wheel 
		 * angle from its initial time and position, and turn it from the
		...
		class ActiveObject : public Ceylan::TextDisplayable
		{
			...
			public:
				 ...
				 * waited by the scheduler until the active object is 
				 ...
				 * programmed objects, since their arguments are almost the
				 * same.
				 *
				 */
				...
				/**
				 * Constructor of a object activated only on specifically
				 * programmed simulation steps. 
				 *
				 * @param triggeringStates is the list of simulation ticks 
				 * where the activation should occur. 
				 * The object does not take ownership of this list, it 
				 * will make a copy of it.
				 *
				 * @param absolutlyDefined tells if the specified steps are
				 * defined absolutly, or relatively to the current 
				 * simulation step.
				 *
				 * @param policy allows to choose a scheduling policy, 
				 * according to the quality of service this active object
				 ...
				 *
				 * @note Do not mix up this constructor with the one for
				 * periodic objects, since their arguments are almost the same.
				 *
				 */
				...
				/**
				 * Constructor of a object activated only on a specific
				 * simulation step. 
				 *
				 ...
				 *
				 * @param absolutlyDefined tells if the specified steps are
				 * defined absolutly, or relatively to the current simulation
				 * step. No default value can be given for that parameter, 
				 * since there would be an ambiguity with first constructor
				 ...
				 * unsubscribing of this object from the scheduler.
				 *
				 */
				 ...
				 *
				 * @note A null (0) value denotes a pure programmed active
				 * object.
				 *
				 */
				 ...
				 * object, expressed in simulation ticks.
				 *
				 * @note An object will be considered as a
				 * periodically-activated one iff its period is non-zero. 
				 * Zero means not activated on a regular basis thanks to
				 * periodic slots.
				 *
				 */
				 ...
				 *
				 * @throw SchedulingException if the object had no programmed
				 * activations.
				 *
				 * @see hasProgrammedActivations to check for programmed
				 * activations before.
				 *
				 */
				...
				/**
				 * Ensures that the programmed ticks for this object are 
				 * unique, which means that, after this method call, this 
				 * object will not be scheduled multiple times for any given
				 * simulation tick.
				 *
				 * @note One can wish an object is scheduled multiple times 
				 * for a given simulation tick, so having unique elements 
				 * in programmed lists is an option.
				 *
				 */
				 ...
				 * this active object.
				 *
				 * @note Called by the schedule, when registering this 
				 * active object.
				 *
				 */				
				virtual void setBirthTime( 
				...
				virtual void onImpossibleActivation( 
						Events::SimulationTick missedStep ) 
					throw( SchedulingException ) ;
			...
			protected:		
				 ...
				 * offset from the moment the object is registered).
				 *
				 * Is true by default.
				 *
				 */
				 ...
				 * time.
				 *
				 * @note the date of birth is neither the creation time 
				 * of this active object (since it could be created even 
				 * before the scheduler exists), nor the time where this 
				 * object is scheduled for the first time, but the time it 
				 * was registered to the scheduler.
				 *
				 */
