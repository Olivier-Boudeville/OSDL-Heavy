...
namespace OSDL
{
	...
	namespace Video
	{
		...
		/*
		 * The scheduler fallbacks to the video module if no renderer is being
		 * used.
		 *
		 */
		class VideoModule ;
	...
	namespace Engine
	{
		 ...
		 * period, expressed in simulation time
		 *
		 *  - objects whose activation is specifically determined, as a set of
		 * simulation ticks
		 *
		 * These ticks are defined relatively to the moment when they have been
		 * declared to the scheduler, or as absolute ticks.
		 *
		 * @note For the application objects, the only relevant and useful unit
		 * for time with respect to scheduling is the simulation time, expressed
		 * in Events::SimulationTick.
		 *
		 ...
		 *
		 * Therefore they are only in soft real-time, and it is the job of the
		 * scheduler to ensure that everything is done on time, on a best effort
		 * basis.
		 *
		 * Whenever a failure to meet a deadline occurs, the impacted objects
		 * are provided with a means of being noticed, in order to be able to
		 * take any counter-measures they can.
		 *
		 * The scheduler has two main data structures:
		 *
		 *  - the 'periodic' one, which is composed of lists of active objects
		 * for each used periodicity, gathered thanks to periodic slots
		 *
		 *  - the 'programmed' one, which allows objects with non periodic
		 * behaviour to plan themselves their activation at specific simulation
		 * ticks
		 *
		 * Our scheduler can follow two different behaviours:
		 *
		 *  - the default one, which focuses on interactive applications, trying
		 * to give the user the best possible logic and, if possible, framerate
		 * and input polling, on soft real time (best effort)
		 *
		 *  - the screenshot mode, where the rendering targets a particular
		 * framerate, and generates PNG screenshots meant to be gathered in a
		 * movie (ex: MPEG)
		 *
		 * This is a batch (non-interactive) mode, where the duration necessary
		 * to update the logic and the frames does not matter: no deadline is to
		 * be respected in this mode, each tick will take exactly the time
		 * needed to be processed
		 *
		 * Choosing a high logic frequency, and moreover higher than the
		 * rendering frequency, is the way to go, since it allows to get rid of
		 * interpolation and resampling issues.
		 *
		 * Clearly 33 Hz is the very least logic frequency one should use, and
		 * 100 Hz or higher frequencies should be preferred.
		 *
		 ...
		 * simulation tick.
		 *
		 * Do not try to depend on any real time measurement, since it would
		 * bypass the efforts of the scheduler to provide the application with a
		 * virtual fixed-rate logic time, and make it fail.
		 *
		 * For the rendering, beware that some graphic primitives wait for
		 * vsync, which may freeze the process until the remaining part of the,
		 * say, 1/60th of second is reached.
		 *
		 ...
		 * shearing.
		 *
		 * Conversely, the scheduler could take this behaviour into account and
		 * synchronize itself on the display refresh rate.
		 *
		 * It would have to manage a decrease in the frame rate if the scene
		 * complexity comes too close to have a rendering duration of the magic
		 * times (1/60 of second, 1/30 etc.): because of random variations, some
		 * frames would go slightly over or under the duration, leading to an
		 * irregular framerate which may be visually disturbing.
		 *
		 ...
		 * operations.
		 *
		 * Rendering is obtained thanks to a renderer, if available, which is
		 * called at each rendering tick.
		 *
		 * If no renderer is being used, then the video module will be used, and
		 * requested to redraw at each rendering tick.
		 *
		 * The scheduled objects are in this case responsible for the updating
		 * of the screen surface.
		 *
		 * @fixme add a means of knowing whether hardware synch with refresh
		 * rate is possible and, if so, use it not to schedule instead of the
		 * scheduler, but to make the scheduler start at the right time, so that
		 * it keeps naturally the synchronisation with the refresh rate.
		 *
		 ...
		 *
		 * The input loop provided by the SDL back-end runs at 100 Hz, so higher
		 * frequencies would be useless.
		 *
		 * @note With default settings, simulation frequency granularity should
		 * be less than + or -3 Hz on average.
		 *
		 * @note The Scheduler is meant to be used as a singleton: having
		 * multiple scheduler instances in use would be meaningless, and they
		 * would probably fail collectively.
		 *
		 ...
		 * at least the simulation evaluation, since there may be numerous
		 * active objects that could take advantage of hyperthreading, dual
		 * cores, etc.
		 *
		 * For the simulation tick, there could be too a two-pass algorithm:
		 * first compute, but do not apply, all the newer states of all objects,
		 * then, second pass, apply them.
		 *
		 * The problem is indeed that an object whose state depends on the state
		 * of other objects should take them into account at time t, when
		 * computing state at t+1, in their state at t, and not t+1.
		 *
		 * If all objects were to be updated in a single row, the first ones
		 * would be already at t+1 while the remaining ones would have to be
		 * updated from t to t+1.
		 *
		 ...
		 * where two objects became mutually dependent), no right order can be
		 * found.
		 *
		 ...
		 * other processes) would be probably necessary, since for example the
		 * Linux kernel seems to take control away for a rather long while when
		 * it detects a program running at 100%.
		 *
		 * However it would involve waiting regularly for 1ms (kernel 2.6) or
		 * 10ms (kernel 2.4), and many skips would occur.
		 *
		 * The user can choose which behaviour to retain by selecting the kind
		 * of idle calls that should be performed.
		 *
		...
		class OSDL_DLL Scheduler : public Ceylan::Object
		{
			...
			public:
				 ...
				 * rendering tick, either to actually render, or to be notified
				 * of a rendering skip.
				 *
				 * @param newRenderer the renderer to be used from now.
				 *
				 * @note Will delete any previously existing renderer, and will
				 * take ownership of the specified renderer.
				 *
				 */
				...
				/**
				 * Tells whether the application should run interactively, in
				 * real time (if <b>on</b> is false), or if it should run in
				 * snapshot mode, with a virtual fixed framerate (as specified
				 * as <b>frameFrequency</b>), with logic and rendering taking
				 * all the time they need.
				 *
				 * In this mode, even if the logic and the rendering tasks last
				 * for two hours for each virtual second, the resulting set of
				 * PNG files is made so that these files can be gathered to form
				 * an animation with, for instance, 25 frames per second.
				 *
				 * When this animation will be played, the logic will seem to
				 * have acted with respect to the planned framerate, as if in
				 * real time.
				 *
				 * @param on tells whether frames are to be rendered to file
				 * (iff on), or if the application should run interactively.
				 *
				 * @param frameFilenamePrefix determines which prefix will be
				 * used to name successive screenshots, in the form:
				 ...
				 *
				 * At least 25 frames per second is recommended to be able to
				...
				/**
				 * Defines how many microseconds an engine tick should last, and
				 * updates accordingly the simulation and rendering ticks.
				 *
				 ...
				 *
				 * @note When changing this setting, in most cases the scheduled
				 * objects are to be changed or updated.
				 *
				...
				/**
				 * Returns the number of engine ticks which correspond to one
				 * simulation tick.
				 *
				 * @note It allows to take into account the round-off that had
				 * to be performed by the setSimulationFrequency method.
				 *
				 */
				...
				/**
				 * Sets the rendering frequency, by evaluating the number of
				 * engine ticks to be associated to one rendering tick.
				 *
				 * @param frequency the new rendering frequency.
				 *
				 * @throw SchedulingException if requested frequency is superior
				 * to engine frequency.
				 *
				 ...
				 *
				 * @note It allows to take into account the round-off that had
				 * to be performed by the setRenderingFrequency method.
				 *
				 */
				...
				/**
				 * Sets the screenshot frequency, by evaluating the number of
				 * engine ticks to be associated to one screenshot tick for
				 * <b>frequency</b> to be met.
				 *
				 ...
				 *
				 * @note It allows to take into account the round-off that had
				 * to be performed by the setScreenshotFrequency method.
				 *
				 */
				...
				/**
				 * Sets the input polling frequency, by evaluating the number of
				 * engine ticks to be associated to one input tick.
				 *
				 * @throw SchedulingException if requested frequency is superior
				 * to engine frequency.
				 *
				 ...
				 *
				 * @note It allows to take into account the round-off that had
				 * to be performed by the setInputPollingFrequency method.
				 *
				 */
				 ...
				 * yield some CPU time so that the OS does not consider it as
				 * too CPU-hungry, and does not take counter-measures against
				 * it.
				 *
				 * @note Ensure that the maximum time spent in the idle callback
				 * is negligible compared to the period corresponding to the
				 * maximum engine frequency being set, so that it does not
				 * result in a too heavy load which would make the scheduler
				 * fail constantly because of its idle function.
				 *
				 * @param idleCallback the idle callback, which can be set to
				 * null (0) to specify no idle callback is wanted.
				 *
				 * @param callbackData the user-supplied data pointer that the
				 * idle callback will be given, if not null.
				 *
				 * @param callbackExpectedMaxDuration the maximum duration, in
				 * microseconds, expected for this idle call-back. Helps the
				 * scheduler enforcing its target frequency. If this value is
				 * null, the idle callback will be launched once immediately
				 * (during the call of this method), and the measured duration,
				 * increased of 20%, will be kept as upper bound.
				 *
				 * Any prior callback or callback data will be replaced by the
				 * one specified.
				 *
				 */
				...
				/**
				 * Returns the number of input ticks corresponding to specified
				 * duration.
				 *
				 * @example If scheduling the inputs at 25Hz, a 2000 ms duration
				 * will correspond to 50 input ticks.
				 *
				 ...
				 * periodic and that determined that they should unregister from
				 * the scheduler.
				 *
				...
				/**
				 * Registers specified periodical active object in scheduling
				 * slots.
				 *
				 * @param toRegister the periodical active object to register to
				 * the scheduler.
				 *
				 */
				...
				/**
				 * Launches the schedule loop, which behaves as a specialized
				...
				/**
				 * Requests the scheduler to stop its activities at the end of
				 * current engine tick.
				 *
				 * @note After this call, the engine may still schedule some
				 * ticks (simulation, input or rendering) if they were remaining
				 * for the current engine tick and/or if ticks were to be
				 * skipped. So most models should include guards to avoid
				 * tear-down operations to be performed more than once.
				 *
				 */
				virtual void stop() ;
				...
				/**
				 * Registers a new stop callback, which will be called each time
				 * the scheduler stops.
				 *
				 * @param stopCallback the callback to register, which can be
				 * either a function or a static method; it can be set to null
				 * (0) to specify no stop callback is wanted.
				 *
				 * @param callbackData the user-supplied data pointer that the
				 * stop callback will be given, if not null.
				 *
				 */
			   ...
			   /**
				* Returns the one and only one scheduler instance, that should
				...
				*/
				 ...
				 * callback triggered by the GUI, so that the program flow can
				 * go past the entering in the main loop.
				 *
				 ...
				 *
				 * Most interactive applications should be satisfied with this
				 * setting.
				 *
				 */
				 ...
				 *
				 * @note A value of 40 Hz (40 frames per second) implies a 25 ms
				 * scheduling.
				 *
				 */
				 ...
				 * purpose of being gathered by a movie with 25 images per
				 * second.
				 *
				 ...
				 *
				 * @note A value of 20 Hz (20 input polling per second) implies
				 * a 50 ms scheduling.
				 *
				 */
			...
			protected:
				 ...
				 *
				 * @throw SchedulingException if the scheduler could not be
				 * returned, for example if no subsecond sleep is available on
				 * this platform.
				 *
				 */
				explicit Scheduler() ;
				...
				/**
				 * Virtual destructor, any existing renderer is deleted whereas
				 * no active object is destroyed.
				 *
				 * @note Protected to ensure the singleton pattern is respected.
				 *
				 */
				...
				/**
				 * Actual scheduling method used for batch applications, which
				 * do not care about keeping any user-defined pace.
				 *
				 * @param pollInputs tells whether inputs should be polled
				 * nevertheless.
				 *
				 * @note The program takes all the time it needs to perform its
				 * tasks, so that no simulation nor rendering tick is skipped,
				 * even if the whole processing takes significantly longer.
				 *
				...
				/**
				 * Returns the value that <b>should</b> be the engine tick, if
				 * no scheduling skip had happened.
				 *
				 * This engine tick is computed from the current time since the
				 * beginning of scheduling, which is obtained thanks to a call
				 * to a precise actual time measurement minus the offset
				 * corresponding to the starting time of the scheduling.
				 *
				...
				/**
				 * Takes care of the simulation by making live relevant active
				 * objects, when a simulation tick is reached.
				 *
				 * @param current the simulation tick to perform, for objects to
				 * be able to behave appropriatly, for example for first
				 * behaviour approach (stateless objects).
				 *
				 */
				 ...
				 *
				 * @param current the simulation tick to perform, for programmed
				 * objects to know to which programmed tick this activation
				 * corresponds.
				 *
				 */
				 ...
				 * activating the relevant ones.
				 *
				 * @param currentSimulationTick the simulation tick to perform,
				 * for objects to be able to behave appropriately, for example
				 ...
				 *
				 * @param currentSimulationTick the simulation tick to be passed
				 * to the activated objects.
				 *
				 * @param objectList the list of the programmed active objects
				 * to execute in turn.
				 *
				 */
				 ...
				 * specified simulation tick, which will have therefore to be
				 * skipped.
				 *
				 * By default, this method calls on all active objects whose
				 * simulation tick has been skipped their onSkip method.
				 *
				 * @note If an object is registered multiple times for a given
				 * skipped simulation tick (ex: multiple periodic and/or
				 * programmed activations), then its onSkip method will be
				 * called as many times as it should have been activated.
				 *
				 * @note Skipping methods should be, on average, faster than
				 * their normally scheduled counterparts (onNextTick), otherwise
				 * the scheduler will have no chance of keeping up with the real
				 * time.
				 *
				 */
				 ...
				 * Overridable method called when this scheduler detects that
				 * processing power was not high enough to perform on time
				 * specified rendering tick, which will have therefore to be
				 * skipped.
				 *
				 * By default, this method does nothing except complaining in
				 * the warning channel.
				 *
				 * @note Skipping methods should be, on average, faster than
				 * their normally scheduled counterparts, otherwise the
				 * scheduler will have no chance of keeping up with the real
				 * time.
				 *
				 */
				 ...
				 * Overridable method called when this scheduler detects that
				 * processing power was not high enough to perform on time
				 * specified input tick, which will have therefore to be
				 * skipped.
				 *
				 * By default, this method does nothing at all. As only the
				 * freshest inputs are generally useful, most applications will
				 * leave this method as a do-nothing one.
				 *
				 * @note Skipping methods should be, on average, faster than
				 * their normally scheduled counterparts, otherwise the
				 * scheduler will have no chance of keeping up with the real
				...
				/**
				 * Called whenever the scheduler does not succeed in keeping up
				 * the pace with what the application demanded: if the runtime
				 * resources are not high enough, then the scheduler may fail
				 * regularly and progressively fill up its delay bucket.
				 *
				 ...
				 * error log channel if below ShutdownBucketLevel, and will stop
				 * the scheduler if higher.
				 *
				...
				/**
				 * Adds a programmed activation for <b>objectToProgram</b> at
				 * simulation tick <b>targetTick</b>.
				 *
				 * @param objectToProgram the object whose activation is being
				 * programmed.
				 *
				 * @param targetTick the simulation tick when specified object
				 * should be activated.
				 *
				 */
				 ...
				 *
				 * @throw SchedulingException if no slot could be found for this
				 * period.
				 *
				 */
				...
				/**
				 * Returns the internal periodic slot corresponding to specified
				 * period.
				 *
				 ...
				 * start-up.
				 *
				 * The dynamically created ones will have their birth time set
				...
				/**
				 * Returns a textual description of the ticks that are currently
				 * programmed.
				 *
				 */
				 ...
				 * deactivated).
				 *
				 * @note Screenshot mode is also known as 'no deadline' mode, as
				 * opposed to 'best effort' mode, which is in soft real time.
				 *
				 */
				...
				/**
				 * Records the user-defined screenshot frequency, which is the
				 * targeted number of frames per second.
				 *
				 * Useful if the corresponding period had to be computed again
				 * after the engine tick duration changed.
				 *
				 */
				...
				/**
				 * Stores the period corresponding to the planned framerate for
				 * the movie that would be generated, if screenshot mode is
				 * activated.
				 *
				 * Movie periods, which are the duration between two successive
				 * screenshots, are expressed in engine ticks.
				 *
				 */
...
#pragma warning( disable: 4251 )
				...
				/**
				 * A list containing all available periodic slots, ordered by
				 * increasing periods.
				 *
				 * @note This is a list of pointers and not of PeriodicSlot
				 * instances since adding a new slot would otherwise result in a
				 * call to a copy constructor we do not want to define.
				 *
				 */
				...
				/**
				 * A list containing all the active objects (either periodical
				 * or programmed) that were registered before the scheduler is
				 * started.
				 *
				 ...
				 *
				 * Each key of the map is a specific simulation tick, whereas
				 ...
				 * 0(1)), insertion times are much faster, (0(n.log n) instead
				 * of 0(n)).
				 *
				 * As there may be numerous programmed triggers (ex: for playing
				 * sound delayed by distance), the map might be helpful here,
				 * although an ordered list might probably be more efficient.
				 *
...
#pragma warning( pop )
				 ...
				 *
				 * Useful to determine how much time have elapsed since last
				 * schedule action.
				 *
				 * @note Under normal circumstances (not too heavy load), the
				 * current engine tick multiplied by the engine tick duration
				 * should correspond to the current user time, with the offset
				 * of the library initialization time t0: (current time) - t0 =
				 * ( current engine tick ) * ( engine tick duration )
				 *
				 * The scheduler has for purpose to enforce this property, but
				 * insufficient processing resources can make the engine time
				 * drift: hence one should not rely on this property being
				 * verified.
				 *
				 ...
				 * simulation action.
				 *
				 * @note Under ideal circumstances (not too heavy load), the
				 * current simulation tick multiplied by the simulation tick
				 * duration should correspond to the current user time, with the
				 * offset of the library initialization time t0: (current time)
				 * - t0 = ( current simulation tick ) * ( simulation tick
				 * duration )
				 *
				 * The scheduler has for purpose to enforce this property, but
				 * insufficient processing resources can make the simulation
				 * time drift: one should not rely on this property being
				 * verified.
				 *
				 * Therefore, this data member is the only one that is
				 * authoritative (the only actual value).
				 ...
				 * rendering action.
				 *
				 * @note Under idel circumstances (not too heavy load), the
				 * current rendering tick multiplied by the rendering tick
				 * duration should correspond to the current user time, with the
				 * offset of the library initialization time t0: (current time)
				 * - t0 = ( current rendering tick ) * ( engine rendering
				 * duration )
				 *
				 * The scheduler has for purpose to enforce this property, but
				 * insufficient processing resources can make the rendering time
				 * drift: one should not rely on this property being verified.
				 *
				 * Therefore, this data member is the only one that is
				 * authoritative (the only actual value).
				 *
				 ...
				 *
				 * @note Under ideal circumstances (not too heavy load), the
				 * current input tick multiplied by the input tick duration
				 * should correspond to the current user time, with the offset
				 * of the library initialization time t0: (current time) - t0 =
				 * ( current input tick ) * ( engine input duration )
				 *
				 * The scheduler has for purpose to enforce this property, but
				 * insufficient processing resources can make the input time
				 * drift: one should not rely on this property being verified.
				 *
				 * Therefore, this data member is the only one that is
				 * authoritative (the only actual value).
				...
				/**
				 * Duration between two successive simulation ticks, expressed
				 * in engine ticks.
				 *
				 */
				...
				/**
				 * Duration between two successive rendering ticks, expressed in
				 * engine ticks.
				 *
				 */
				...
				/**
				 * Duration between two successive input ticks, expressed in
				 * engine ticks.
				 *
				 */
				 ...
				 * callback.
				 *
				 * Helps the scheduler to respect its expected pace while still
				 * issuing idle calls.
				 *
				 */
				...
				/**
				 * Records how many simulation ticks were recovered since the
				...
				/**
				 * Records how many simulation ticks were missed since the
				 * scheduler was started.
				 *
				 */
				...
				/**
				 * Records how many rendering ticks were recovered since the
				 * scheduler was started.
				 *
				 */
				...
				/**
				 * Records how many rendering ticks were missed since the
				 * scheduler was started.
				 *
				 */
				...
				/**
				 * Records how many input ticks were recovered since the
				 * scheduler was started.
				 *
				 */
				...
				/**
				 * Records how many input ticks were missed since the scheduler
				 * was started.
				 *
				 */
				 ...
				 *
				 * @note Used mostly when the scheduler has to poll inputs
				 * during input tick.
				 *
				 */
				 ...
				 * renderer.
				 *
				 * In this case, at each rendering tick the scheduler calls the
				 * redraw method of the video module.
				 *
