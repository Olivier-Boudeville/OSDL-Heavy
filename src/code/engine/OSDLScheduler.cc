 ...
 *
 * Each of these three actions can specify at which frequency it should be run,
 * knowing that the scheduler runs itself at a fundamental maximal frequency of
 * 1000Hz.
 *
 * When running in best effort mode, the scheduler will do its best to respect
 * the assigned frequencies and, if possible, trigger idle calls, to decrease
 * the system load.
 *
 * Actions will have also different priorities, as for example skipping
 * simulation ticks should be avoided at all costs, whereas rendering ticks are
 * less crucial.
 *
 * Missed deadlines can be re-planned if being not too late, and according to
 * the aforementioned priorities.
 *
 * The active objects can be periodical ones and/or programmed ones, i.e. they
 * will be activated every N simulation ticks and/or at specific ticks,
 * specified by the object.
 *
 * To manage periodical objects, an ordered list of slots is used: each of these
 * slot will manage all objects which have the same period.
 *
 * A slot for period N will have N sub-slots. Objects of period N will be
 * dispatched among these subslots to smooth the load, to avoid having too
 * numerous objects activated during one simulation tick.  This scattering does
 * not prevent the scheduler to jump over several ticks, as anyway it will most
 * probably have to schedule objects every simulation tick.
 *
...
void Scheduler::setRenderingFrequency( Hertz frequency )
{
	 ...
	 * duration is 1000 microseconds, there are 1E6/(40*1000) = 25 engine ticks
	 * for each rendering tick.
	 *
	 */
...
void Scheduler::setScreenshotFrequency( Hertz frequency )
{
	 ...
	 * duration is 1000 microseconds, there are 1E6/(25*1000) = 40 engine ticks
	 * for each screenshot tick.
	 *
...
void Scheduler::setInputPollingFrequency( Hertz frequency )
{
	 ...
	 * duration is 1000 microseconds, there are 10E6/(20*1000) = 50 engine ticks
	 * for each input tick.
	 *
	 */
	...
	PeriodicalActiveObject & toUnregister )
{
	...
	/*
	 * This periodical object must be in one of the subslots of the slot in
	 * charge of its periodicity.
	 *
	 * As an active object is most generally removed by another one, we know
	 * that the scheduler is in the process of iterating over active objects.
	 *
	 * Therefore we cannot remove that object from a subslot we could be already
	 * iterating over: this would invalidate the used iterator and result in a
	 * runtime error.
	 *
	 * Instead we just set the corresponding pointer in subslot to null
	 * (i.e. its list element carries now a null pointer), knowing that, when
	 * iterating in a subslot, if we find a null pointer then we remove the
	 * corresponding list element.
	 *
	 * That way the two-phase removal does not lead to incorrect iterator
	 * management.
	 *
	 * Only trade-off: a mandatory check at each element that its pointer is not
	 * null, whereas the only problematic case is when a scheduled object leads
	 * to the removal of another one *in the same subslot*.
	 *
	 */
	...
	ProgrammedActiveObject & objectToRegister )
{
	...
	if ( ! objectToRegister.areProgrammedActivationsAbsolute() )
	{
		...
		/*
		 * else: we assume that when the scheduler will start, it will start at
		 * simulation tick #0.
		 *
		 * Otherwise the programmed objects could be planned only at start-up,
		 * to take into account any non-null initial simulation tick.
		 *
		 */
	...
	ProgrammedActiveObject & objectToUnregister )
{
	...
	/*
	 * This programmed object may be registered in the list of objects to be
	 * scheduled during the current tick of the scheduler.
	 *
	 * As we cannot change the structure of a list while iterating on it, we
	 * just replace in the corresponding node of the list a pointer to this
	 * object to unregister by a null pointer.
	 *
	 * Although we could do this only for the hashtable entry corresponding to
	 * the current tick, we do it for all remaining ticks.
	 *
	 * Only trade-off: a mandatory check at each element that its pointer is not
	 * null, whereas the only problematic case is when a scheduled object leads
	 * to the removal of another one *in the list for the current tick*.
	 *
	 * Anyway this check cannot be avoided for that case, thus null pointers can
	 * be set in other ticks than current ones as well.
	 *
	 */
	...
	while ( it != programmed.end() )
	{
		...
		if ( mapIt != _programmedActivated.end() )
		{
			...
			/*
			 * Here we found the list for that tick, removes this object from
			 * that list:
			 *
			 */
...
void Scheduler::scheduleBestEffort()
{
   ...
   */
	...
	if ( _idleCallback == 0 && _idleCallbackMaxDuration == 0 )
	{
		...
		/*
		 * Not artificially increased, as will be updated in the course of the
		 * execution, and we do not want to overestimate that maximum.
	...
	/*
	 * This is the default value for the duration of an idle callback, it will
	 * be modulated according to runtime measures to be more accurate.
	 *
	 * Even under heavy loads, the measured scheduling granularity is stable and
	 * accurate, hence it is a reliable reference. Let's call it Dstart.
	 *
	 * During the scheduler run, the machine can become more loaded than at the
	 * scheduler startup. On average, each atomic sleep will last for longer,
	 * and the scheduler has to adapt. Hence it measures the duration Dlast of
	 * the latest atomic sleep as an indicator for next sleep.
	 *
	 ...
	 * artefact-like Dlast: otherwise the scheduler would remember only the past
	 * artefact and would never trigger an idle call, as it would never have a
	 * next deadline distant enough to allow for such a duration.
	 *
	 * What is true for atomic sleeps is deemed true too for user-supplied idle
	 * callbacks.
	 *
	 */
	 ...
	 * rendering and input times: _currentEngineTick, for an engine tick
	 * duration of 1000 (microseconds), should wrap around if the program runs
	 ...
	 * duration will be set to a too low value, and this will lead to hazardous
	 * initial idle calls.
	 *
	 */
	...
	/*
	 * If OSDL_DEBUG_SCHEDULER is set, run-time informations about the scheduler
	 * will be archived and analyzed after it stopped, like black boxes.
	 *
	 */
	 ...
	 * which are difficult to enforce: we would never have 100 Hz, but often 95
	 ...
	 *
	 *
	 * Overloaded situations are detected thanks to a kind of leaking bucket:
	 * each delay of d ms fills the bucket by an amount that increase with d
	 * (depending on the tolerance being exceeded or not), each elapsed engine
	 * tick makes the bucket leak a bit.
	 *
	 ...
	 *
	 * Higher delays, which would be strongly abnormal, would lead to simulation
	 * skips.
	 *
	 ...
	 * serious: next rendering will replace it, we do not <b>have</b> to force
	 * this particular rendering to happen.
	 *
	...
	/*
	 * A rendering is accepted if it would be no more late than one quarter of
	 * its period:
	 *
	 */
	 ...
	 *
	 * The threshold is quite low, as skipping input ticks is not that serious:
	 * next input tick will replace it, we do not <b>have</b> to force this
	 * particular input polling to happen.
	 *
	...
	/*
	 * An input polling is accepted if it would be no more late than half of its
	 * period:
	 *
	 */
	...
	while ( ! _stopRequested )
	{
		...
		/*
		 * Normally, for each simulation tick, either scheduleSimulation or, if
		 * late, onSimulationSkipped are called.
		 *
		 * Similarly, for each rendering tick, either scheduleRendering or, if
		 * late, onRenderingSkipped are called.
		 *
		 * It is soft real-time because the engine tick is derived directly from
		 * the system clock, it cannot drift.
		 *
		 */
		 ...
		 * (notably, the rendering) last more than the current engine tick, it
			...
			&& delayBucket < ShutdownBucketLevel )
		{
			...
			if ( missedTicks > simulationToleranceTick )
			{
				...
				/*
				 * Missing a simulation deadline is serious, so the bucket needs
				 * to fill quickly if tolerance is exceeded.
				 *
				 ...
				 * would saturate for too long the bucket, whereas the computer
				 * can actually face the average load quite well.
				 *
				 * Hence we chose an increasing delay function which does not
				 * lead to too explosive values for high arguments: the square
				 * root, more precisely f: d -> 35.sqrt( 2 * d ); f(500) is
				 * about 1100.
				 *
				 * Check with gnuplot:
				 * set xrange [0:1000]
				 ...
				 * plot 35 * sqrt( 2 * x )
				 *
				 * We tried also f: d -> 20.log( 2*d ), but the log smoothes too
				 * much the delay curve, which leads to a scheduler stopping too
				 * late.
				 *
				 ...
				 * (far too long), whereas with square root it lasts for about 5
				 * seconds (ok).
				 *
		 ...
		 * delay, but only once, i.e. one rendering will then be performed, but
		 * one only, even if we were late of more than one: this is no point in
		 * performing multiple renderings in a row if nothing has changed
		 * in-between (compensation made not to depend from the rendering
		 * tolerance, that could be arbitrarily high).
		 *
		 */
		 ...
		 * but one only, even if we were late of more than one: this is no point
		 * in performing multiple input polling in a row if nothing has changed
		 * in-between (compensation made not to depend from the input tolerance,
			...
			&& delayBucket < ShutdownBucketLevel )
		{
			...
			/*
			 * Manage all missed input ticks and warn (this is not too serious
			 * skip):
			 *
			 */
			EngineTick missedTicks =
		...
		else if ( nextDeadline <= _currentEngineTick + 1 )
		{
			...
			/*
			 * nextDeadline is this current engine tick, or the next one. In
			 * both cases it is to be handled immediatly, no special jump to
			 * perform:
			 *
			 */
			OSDL_SCHEDULE_LOG(
		...
		/*
		 * Do nothing: wait, first with idle callback (which results in atomic
		 * sleeps by default), then for fine-grain with soft busy waitings.
		 *
		 */
	...
	/*
	 * Check that all simulation ticks were handled one and only one time, one
	 * way or another (scheduled: met or recovered, or skipped).
	 *
	 */
	...
	/*
	 * Check that all rendering ticks were handled one and only one time, one
	 * way or another (scheduled: met or recovered, or skipped).
	 *
	 */
	...
	/*
	 * Check that all input ticks were handled one and only one time, one way or
	 * another (scheduled or skipped):
	 *
	 */
...
EngineTick Scheduler::computeEngineTickFromCurrentTime()
{
	...
	/*
	 * _secondToEngineTick necessary to avoid overflow when duration in seconds
	 * exceeds 4200!
	 *
...
void Scheduler::onSimulationSkipped( SimulationTick skipped )
{
	...
	/*
	 * First, check objects which should have been triggered because of periodic
	 * activation:
	 *
	 */
...
PeriodicSlot & Scheduler::returnPeriodicSlotFor( Events::Period period )
{
		...
		it != _periodicSlots.end();	it++ )
	{
		...
		if ( (*it)->getPeriod() > period )
		{
			...
			/*
			 * No slot already available, create it and insert it <b>before</b>
			 * current one.
			 *
			 */
	...
	/*
	 * Period not found, higher than all others: create it at the end of the
	 * list.
	 *
	 */
	PeriodicSlot * newSlot = new PeriodicSlot( period ) ;
