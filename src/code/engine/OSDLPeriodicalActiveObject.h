...
namespace OSDL
{
	...
	namespace Engine
	{
		...
		/**
		 * Periodical active objects are active objects that are to be scheduled
		 * at a regular pace: for an object whose period is N, the scheduler is
		 * expected to trigger it every N of its fundamental simulation ticks.
		 *
		 * Therefore a periodical object can run at any frequency that is a
		 * submultiple of the scheduler fundamental one.
		 *
		 ...
		 * frequency, and their scheduling should be relatively balanced, from a
		 * simulation tick to another.
		 *
		 * Note that this scattering will not decrease the number of idle calls
		 * that can be performed, as they are to take place between simulation
		 * ticks, which will be scheduled regardless of whether there is a
		 * periodical object to activate.
		 *
		...
		class OSDL_DLL PeriodicalActiveObject : public ActiveObject
		{
			...
			public:
				...
				/**
				 * Constructor of a periodically activated object, which will be
				 * activated with the specified period, expressed directly as a
				 * duration in user time, not as a number of scheduling ticks.
				 *
				 ...
				 * tolerated error between the requested period duration and the
				 * actual one (they may differ to the granularity of scheduling
				 * ticks).
				 *
				 ...
				 * waited by the scheduler until this active object is activated
				 * again. The period must not be null.
				 *
				 * @param autoRegister this object will automatically register
				 * to the scheduler iff true.
				 *
				 * @param policy allows to choose a scheduling policy, according
				 * to the quality of service this active object requires.
				 *
				 * @param weight evaluates how much processing power an
				 * activation of this object is to cost, on average. This helps
				 ...
				 * Virtual destructor.
				 *
				 * @note This object will automatically unsubscribe from the
				 * scheduler if needed.
				 *
				 */
				...
				/**
				 * Unregisters this active object from the existing scheduler to
				 * whom it was supposedly registered.
				 *
				 * @throw SchedulingException if the operation failed.
				 *
				 */
				...
				/**
				 * Sets the period of this object so that it gets activated on
				 * specified frequency, making this object periodically
				 * activated if it was not, or changing its already defined
				 * period otherwise.
				 *
				 * @param newFrequency the desired frequency, expressed in
				 * Hertz.
				 *
				 * @return The actual chosen frequency, which may differ a bit
				 * from specified one since it depends on the simulation tick
				 * granularity.
				 *
				 * Notably, the obtained frequency cannot exceed the specified
				 * simulation frequency for the scheduler.
				 *
				 * The actual frequency is nevertheless chosen as the nearest
				 * possible frequency.
				 *
				 * @note To be able to compute the actual period, expressed in
				 * simulation ticks, from the specified frequency, the scheduler
				 * that will be later used for this object is needed.
				 *
				 * @throw SchedulingException if there was no existing scheduler
				 * available.
				 *
				 */
				...
				/**
				 * Returns the number of the sub-slot this active object is
				 * referenced in.
				 *
				 * This sub-slot is set on registration: the scheduler calls the
				 * onRegistering method of this object.
				 *
				 * @throw SchedulingException if the operation failed, notably
				 * if this object is not stored in a sub-slot already.
				 *
				 */
				 ...
				 *
				 * @note Registering does not always imply object birth, as if
				 * the scheduler is not currently running, then birth will
				 * happen only when it is started.
				 *
				 * @throw SchedulingException if the operation failed.
				 *
				 */
				...
				/**
				 * Sets the birth time, expressed in simulation ticks, of this
				 * active object.
				 *
				 * @note Called by the scheduler, when registering this active
				 * object. This object will compute its exact actual begin tick,
				 * depending on the sub-slot it has been put in.
				 *
				 * @throw SchedulingException if the operation failed.
				 *
				 */
				...
				/**
				 * Returns the number of activations of this periodical object
				 * which corresponds to the specified duration.
				 *
