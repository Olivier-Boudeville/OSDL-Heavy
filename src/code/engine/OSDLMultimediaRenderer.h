...
namespace OSDL
{
	...
	namespace Rendering 
	{
		 ...
		 *
		 * This multimedia renderer delegates all graphical concerns to an internal video renderer,
		 * if any, and all sound concerns to an internal audio renderer, if any.
		 *
		 * This kind of renderer is often used as a root renderer, since it is able to federate all
		 * multimedia needs for the views.
		 *
		...
		class MultimediaRenderer : public Renderer
		{
			...
			public:
				 ...
				 *
				 */
				virtual VideoRenderer & getVideoRenderer() const throw( RenderingException ) ;
				 ...
				 *
				 * If a renderer was already registered, it is deallocated first : this renderer
				 * takes ownership of its sub-renderers and manages their life cycle.
				 *
				 */
				 ...
				 *
				 */
				virtual AudioRenderer & getAudioRenderer() const throw( RenderingException ) ;
				 ...
				 *
				 * This method will manage both the video and audio renderings, should the 
				 * dedicated specialized renderers be available.
				 *
				 ...
				 * if any.
				 *
				 * This method will manage the video rendering, and is to be call by the overall
				 * <b>render</b> method, implemented by all renderers.
				 *
				 * @param currentRenderingTick the rendering tick corresponding to this render
				 * step. If the renderer is called from a basic event loop (no scheduler is used),
				 * then the rendering tick is meaningless and is always zero.
				 ...
				 *
				 * @note This method is used instead of calling directly the internal video 
				 * renderer so that informations can be more easily shared between the multimedia
				 * renderer and its specialized counterparts.
				 *
				 */
				virtual void renderVideo( Events::RenderingTick currentRenderingTick = 0 ) 
					throw() = 0 ;
				
				
				/**
				 * Triggers the actual audio rendering of all views, for specified rendering tick,
				 * if any.
				 *
				 * This method will manage the audio rendering, and is to be call by the overall
				 * <b>render</b> method, implemented by all renderers.
				 *
				 * @param currentRenderingTick the rendering tick corresponding to this render
				 * step. If the renderer is called from a basic event loop (no scheduler is used),
				 ...
				 *
				 * @note This method is used instead of calling directly the internal audio 
				 * renderer so that informations can be more easily shared between the multimedia
				 * renderer and its specialized counterparts.
				 *
				 */
				virtual void renderAudio( Events::RenderingTick currentRenderingTick = 0 ) 
					throw() = 0 ;
			...
			private:
				 ...
				 *
				 */			 
				explicit MultimediaRenderer( const MultimediaRenderer & source ) throw() ;
				 ...
				 *
				 */			 
				MultimediaRenderer & operator = ( const MultimediaRenderer & source ) throw() ;
