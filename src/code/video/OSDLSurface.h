...
namespace OSDL
{
	...
	namespace Video
	{
		...
		/*
		 * No way of forward declaring LowLevelSurface apparently: we would have
		 * liked to specify 'struct LowLevelThing ;' here and in the
		 * implementation file (.cc): 'typedef BackendThing LowLevelThing' but
		 * then the compiler finds they are conflicting declarations.
		 *
		 */
		...
		/**
		 * Mother class of all events sent by a Surface (event source) to its
		 * widgets (listeners).
		 *
		 */
		...
		class OSDL_DLL VideoMemoryLostException : public VideoException
		{
			...
			public:
				...
				/**
				 * Constructor with no message, used when no particular reason
				 * is known.
				 *
				 */
		 ...
		 *
		 * Its display type depends on whether this buffer corresponds to a
		 * back-buffer or to the screen (display buffer) and, in this case, on
		 * whether it is OpenGL-based.
		 *
		 * A Surface is an UprightRectangle for its shape.
		 *
		 * Note that it therefore inherits _width and _height attributes from
		 * UprightRectangle, but they are not of use for Surfaces since the only
		 * values that matter for them are to be read directly from the internal
		 * back-end surface.
		 *
		 * Hence getWidth() and getHeight() should <b>always</b> be used, even
		 * for the implementation of Surface's own methods, instead of _width
		 * and _height.
		 *
		 * The clipping area of a Surface is another UprightRectangle, which
		 * must be entirely contained in its Surface.
		 *
		 * Surfaces are Lockable UpRightRectangle instances.
		 *
		 * They are also Smart resources, which implies they are Mesurable and
		 * Clonable.
		 *
		 * In the future, Surface instances may use smart pointers or reference
		 * counting (AddRef/Release) from Ceylan.
		 *
		 * Surfaces can contain widgets, which they own.
		 *
		 * Surfaces maintain a 'need for redraw' state, which determines whether
		 * a given surface has to be redrawn.
		 *
		 * As soon as a graphical element (be it Surface, Widget or other) needs
		 * to be redrawn, the whole container tree has to be itself rescanned,
		 * because of possibly overlapping elements.
		 *
		 * @see OSDL-usersguide.html for more detailed informations on widget
		 * management.
		 *
		 * Surface will automatically disable locking/unlocking if possible, one
		 * does not therefore need to test it with Surface::mustBeLocked before
		 * locking or unlocking it.
		 *
		 * @note In case the surface is double-buffered, and if everything is
		 * not to be redrawn at each frame, the surface should be drawn twice,
		 * so that both buffers are filled.
		 *
		 ...
		 * surface.
		 *
		 * A surface being a widget container, it is an event source, since it
		 * must register its widgets so that they perform pick-correlation or
		 * get, for example, demand for redraw events.
		 *
		 * Stacking is handled thanks to a list of widgets, which is this event
		 * source's listeners list, listeners being sorted from bottom to top.
		 *
		 ...
		 * iteration on listener ordered list: they should be then drawn from
			...
			public Ceylan::SmartResource
		{
			...
			public:
				...
				/**
				 * Indicates that a surface should use asynchronous blits if
				 * possible.
				 *
				 */
				...
				/**
				 * Indicates that a surface is to use RLE-accelerated colorkey
				 * blits.
				 *
				 */
				...
				/**
				 * Indicates that a <b>display</b> surface is to be full screen,
				 * not windowed.
				 *
				 */
				...
				/**
				 * Indicates that a <b>display</b> surface should have an OpenGL
				 * context.
				 *
				 */
				 ...
				 *
				 * Actually all created surfaces have all bytes set to zero for
				 * efficiency reasons, therefore all pixel colors are zero too,
				 * which explains the difference between the RGB and the RGBA
				 * cases.
				 *
				 * In the latter case, use 'fill(Pixels::Black)' to have all
				 * pixels set to a non-fully transparent color such as
				 ...
				 *
				 * If color masks are all zeros (for instance, if their value is
				 * not specified) then they will be set as RGBA coordinates
				 * convenient for OpenGL, according to the endianness of the
				 * system.
				 *
				 ...
				 *
				 * @param depth if its value is 8 (bits), an empty palette is
				 * allocated for the surface, otherwise a 'packed-pixel' pixel
				 * format is created, using the [RGBA] mask's provided, if any,
				 * or the default masks.
				 *
				 * @param redMask the mask to be used to isolate the red
				 * component.
				 *
				 * @param greenMask the mask to be used to isolate the green
				 * component.
				 *
				 * @param blueMask the mask to be used to isolate the blue
				 * component.
				 *
				 * @param alphaMask the mask to be used to isolate the alpha
				 * component. If non null, the Surface::AlphaBlendingBlit flag
				 * is automatically set.
				 *
				 * @throw VideoException if an error occurred.
				 *
				 * @note If all four color masks are left undefined or are set
				 * to zero, then the masks will be set automatically to the ones
				 * expected by OpenGL for textures.
				 *
				 * @note If an alpha-channel is specified (that is, if alphaMask
				 * is nonzero), then the Surface::AlphaBlendingBlit flag is
				 * automatically set.
				 *
				 * You may remove this flag by calling setAlpha after surface
				 * creation.
				 *
				 */
				 ...
				 *
				 * @param surface the SDL surface to manage, whose ownership is
				 * taken by this OSDL Surface.
				 *
				 * @param displayType tells what is the display type of the
				 * provided surface, i.e. whether it is a backbuffer or a screen
				 * surface, usual or OpenGL-based.
				 *
				...
				/**
				 * Clones this Surface: creates a new Surface, exact copy of
				 * this Surface, but with no data shared with it at all.
				 *
				 * The display type of the clone will be back-buffer.
				 *
				 * @throw VideoException on failure, such as the impossibility
				 * of creating the new surface, or to blit the former one.
				 *
				 * @note Returns 'Clonable' instead of 'Surface' since adjusting
				 * pointers for covariant returns is not implemented yet with
				 * gcc (3.3.5).
				 *
				 */
				...
				/**
				 * Sets this surface so that it corresponds to the supplied SDL
				 * surface.
				 *
				 * @param newSurface the new surface to be managed
				 *
				 * @param displayType tells whether this new surface is a
				 * backbuffer or a screen surface.
				 *
				...
				/**
				 * Returns the display type (back-buffer, screen, etc.)  of this
				 * Surface.
				 *
				 */
				...
				/**
				 * Sets the new display type (back-buffer, screen, etc.)  of
				 * this surface.
				 *
				 */
				...
				/**
				 * Converts the internal surface to the pixel format of the
				 * display, so that any blit of this surface to the screen is
				 * quicker.
				 *
				 ...
				 *
				 * This method can be used to convert a colorkey to an alpha
				 * channel, if the ColorkeyBlit flag is already set on this
				 * surface and if alphaChannelWanted is true.
				 *
				 * The generated surface will then be transparent
				 * (alpha=AlphaTransparent) where the pixels match the colorkey,
				 * and opaque (alpha=AlphaOpaque) elsewhere.
				 *
				 * @throw VideoException if no internal surface is available, if
				 * the conversion failed or if there is not enough memory.
				 *
				 */
				 ...
				 * per-surface alpha value and/or enables or disables alpha
				 * blending.
				 *
				 * @param flags can be an OR'd combination of the following two
				 * options, one of these options, or 0 (none):
				 *
				 *  - Surface::AlphaBlendingBlit
				 *  - Surface::RLEColorkeyBlit
				 *
				 * @param newAlpha is the per-surface alpha value. A surface
				 * needs not to have an alpha channel to use per-surface alpha,
				 * and blitting can still be accelerated with
				 * Surface::RLEColorkeyBlit.
				 *
				 * @note 0 (AlphaTransparent) is considered fully transparent
				 * and 255 (AlphaOpaque) is considered fully opaque.
				 *
				 * @note The per-surface alpha value of 128 is considered a
				 * special case and is optimised, so it is much faster than
				 ...
				 *
				 * This is done by checking the corner pixels, supposedly set to
				 * this colorkey.
				 *
				...
				/**
				 * Returns the pixel color of the colorkey of this surface, as
				 * attempted to be guessed from the actual pixel colors (alpha
				 ...
				 *
				 * The key must be defined accoring to the same pixel format as
				 * the surface.
				 *
				 * @param flag can be a OR'ed combination:
				 *
				 * - if Surface::ColorkeyBlit is set, then the key is the
				 * transparent pixel value in the source image of a blit
				 *
				 * - if Surface::RLEColorkeyBlit is set, RLE acceleration will
				 * be used when blitting this surface. The RLE encoding will be
				 * performed the first time the surface is blitted or when it
				 * will be converted to display format
				 *
				...
				/**
				 * Converts this surface, which must use a color key, so that it
				 * uses an alpha channel instead.
				 *
				 * This method uses 'convertToDisplay'.
				 *
				 * @throw VideoException if no internal surface is available, if
				 * it does not use color key, if the conversion failed or if
				 ...
				 *
				 * @param numberOfColors the number of colors that should be
				 * copied, starting from <b>startingColorIndex</b>. If zero, all
				 * remaining color definitions from newPalette will be taken.
				 *
				 * @param targetedPalettes a flag which tells whether logical
				 * palette, physical one or both (OR'ed) should be updated.
				 *
				 * @return true if all colours were set exactly as requested,
				 * false otherwise (not all the color entries were set exactly
				 * as given).
				 *
				 ...
				 * best with the pixel of the surface the method has been called
				 * on.
				 *
				 * @param palette the target palette for the color reduction.
				 *
				 * @param manageColorkey tells whether a colorkey should be
				 * taken care of: if true, an attempt to guess the colorkey of
				 * this surface will be performed.
				 *
				 * If it succeeds then during color-reduction this guessed
				 * colorkey will be matched with the supposedly declared palette
				 * colorkey, no matter how different there colors are. This way
				 * colorkey information is kept existing and separated from the
				 * other colors.
				 *
				 * @throw VideoException if the operation failed, included if
				 * manageColorkey is true but no colorkey could be guessed or if
				 * it is guessed but there is no colorkey registered in target
				 * palette.
				 *
				 ...
				 * there is no pixel format to retrieve.
				 *
				 */
				 ...
				 *
				 * If the new width is different from the current one, triggers
				 * a resize that will blank (in pure black) this surface and set
				 * its 'needs redraw' state to true.
				 *
				 */
				 ...
				 *
				 * If the new height is different from the current one, triggers
				 * a resize that will blank (in pure black) this surface and set
				 * its 'needs redraw' state to true.
				 *
				 */
				 ...
				 * specified ones.
				 *
				 * If the new dimensions are the same as the current ones, then
				 * nothing special will be done, surface stays as is.
				 *
				 * Otherwise, the surface will be resized and, depending of the
				 * 'scaleContent' flag, its content will be scaled (if true) or
				 * lost and replaced by a pure black surface, with the 'needs
				 * redraw' state set to true (if false).
				 *
				 * @param newWidth the new surface width.
				 *
				 * @param newHeight the new surface height;
				 *
				 * @param scaleContent tells whether content should be scaled,
				 * or lost. If true, current content will be scaled, and
				 * antialiased if the Video module state machine says so.
				 *
				 * @throw VideoException if the operation failed or is not
				 * supported.
				 *
				...
				/**
				 * Returns this surface's overall color depth, <b>bytes</b> per
				 * pixel.
				 *
				...
				/**
				 * Returns this surface's video buffer, i.e. its whole set of
				 * pixels.
				 *
				 */
				 ...
				 *
				 * Can be used for blanking a surface, if the background color
				 * is chosen.
				 *
				 ...
				 *
				 * This method clears the viewport for OpenGL surfaces (both the
				 * color buffer and the depth buffer), otherwise performs a mere
				...
				/**
				 * Returns the result of a vertical flip of this source surface:
				 * the result will contain the original image mirrored according
				 * to a vertical line splitting this surface into a left and
				 * right area of equal size.
				 *
				 * Something looking as '>' would be flipped into '<'.
				 *
				 * @return a new surface whose pixels are the ones of this
				 * left-untouched surface, the pixels being vertically flipped.
				 *
				 * @note The returned surface will have to be deallocated by the
				 * caller.
				 *
				 * @throw VideoException if the operation failed or is not
				 * supported.
				 ...
				 * surface: the result will contain the original image mirrored
				 * according to an horizontal line splitting this surface into a
				 * top and bottom area of equal size.
				 *
				...
				/**
				 * Returns a textual description of the pixel of this surface
				 * located at [x;y].
				 *
				 ...
				 *
				 * @return a string containing a description (mainly the RGBA
				 ...
				 * surface, located at [x;y], encoded according to the pixel
				 * format of this surface.
				 *
				...
				/**
				 * Returns the color definition of one particular pixel of this
				 ...
				 * destination pixel. If false, the exact specified color will
				 * be put in target pixel, instead of being blended with it.
				 *
				 * @param clipping tells whether point location is checked
				 * against surface bounds. If clipping is activated and the
				 * pixel is outside, nothing is done.
				 *
				...
				/**
				 * Sets the pixel at [x;y] of this surface to the given color,
				 * specified as a color definition (set of RGBA coordinates).
				 *
				 * @param x the abscissa of the point to change.
				 *
				 * @param y the ordinate of the point to change.
				 ...
				 *
				 * @param alpha the alpha coordinate of the pixel to be put with
				 * full precision: the alpha encoded in the converted color may
				 * not be reliable.
				 *
				 * @param blending tells whether the alpha channel must be taken
				 * into account, resulting to alpha blending with the
				 ...
				 * described by <b>colorDef</b> to <b>newAplha</b>, by scanning
				 * all the pixels of this RGBA surface.
				 *
				 * @example if colorDef is [ x; y; z; t ] (t is ignored), then
				 * all the pixels of this surface whose RBG values are equal to
				 * [ x; y ; z; * ] will be set to [ x; y ; z; newAlpha ],
				 * regardless of their previous alpha coordinate.
				 *
				 * @param colorDef the color definition describing the RGB value
				 * whose alpha coordinate has to be replaced.
				 *
				 * @param newAlpha the alpha coordinate that must be assigned to
				 * matching pixels.
				 *
				 * @return false iff something went wrong (ex: this is not a
				 * RGBA surface).
				 *
				 * @note This method is rather expensive (slow) but useful since
				 * blits cannot achieve that effect.
				 *
				 */
				 ...
				 * endpoint drawing mode is set, with specified RGBA color, in
				 * this surface.
				 *
				 ...
				 * (xStart;yStart), included, to point (xStop;yStop), included
				 * if and only if endpoint drawing mode is set, with specified
				 * RGBA color.
				 *
				 * @return false if and only if something went wrong
				 * (ex: surface lock failed).
				...
				/**
				 * Draws a cross centered in <b>center</b>, included in a square
				 * whose length of edge is <b>squareEdge</b>.
				 *
				 ...
				 * @note Clipping is performed.
				 *
				 * @note This cross will be antialiased if antialias mode is
				 * set.
				 *
				 */
				...
				/**
				 * Draws the edges of this surface, with specified color and
				 * edge width.
				 *
				 ...
				 * filled with specified color, or if only its sides should be
				 * drawn.
				 *
				 * @note Locks surface if needed.
				 *
				...
				/**
				 * Draws a circle whose center is (xCenter,yCenter) of specified
				 * radius, with specified RGBA color, on this surface.
				 *
				 * @param filled tells whether the circle should be filled
				 * (disc).
				 *
				 * @param blended tells whether, for each pixel of disc, the
				 * specified color should be blended with the one of the target
				 * pixel (if true), or if the specified color should replace the
				 * former one, regardless of any blending (if false).
				 *
				 ...
				 * radius (totalRadius), filled with specified color
				 * discColorDef, with a ring in it, on its border, i.e. starting
				 * from innerRadius to totalRadius, colored with ringColorDef,
				 * on this surface.
				 *
				 ...
				 * disc, the specified color should be blended with the one of
				 * the target pixel (if true), or if the specified color should
				 * replace the former one, regardless of any blending (if
				 * false).
				 *
				 * @return false if and only if something went wrong
				 * (ex: surface lock failed).
				 ...
				 *
				 * @note Locks surface if needed, cipping is performed, disc
				 ...
				 * specified horizontal and vertical radii, with specified RGBA
				 ...
				 * color, on this surface, starting from angleStart, stopping to
				 * angleStop, both expressed in degrees.
				 *
				 ...
				 * pointers'.
				 *
				 * @param x the abscissa the polygon should be translated of, on
				 * screen.
				 *
				 * @param y the ordinate the polygon should be translated of, on
				 * screen.
				 *
				...
				/**
				 * Draws a grid, with specified space between columns and rows
				 ...
				 *
				 * @param fillBackground tells whether a background should be
				 * filled.
				 *
				 * @param backgroundColor the color of the background, if
				 * activated.
				 *
				 ...
				 *
				 * Blits from a RGBA Surface with AlphaBlendingBlit set to
				 * another RGBA surface will keep the alpha of the destination
				 * surface.
				 *
				 ...
				 *
				 * Hence one might set the alpha of the destination surface to
				 * targeted value before the blit.
				 *
				 * @param targetSurface the destination surface this surface
				 * will be blitted to, starting at [0;0] in the destination
				 * surface: both upper left corners will correspond.
				 *
				 * @throw VideoMemoryLostException if either of the surfaces was
				 * in video memory, and if the video memory was lost: it then
				 * should be reloaded with content and re-blitted. Otherwise, if
				 * blit went wrong differently, throw a commented
				 * VideoException.
				 *
				 ...
				 * will be blitted to.
				 *
				 * @param x abscissa of the destination surface where this
				 ...
				 * were in video memory, and if the video memory was lost: it
				 * then should be reloaded with content and re-blitted.
				 ...
				 * will be blitted to.
				 *
				 * @param location the point of the destination surface where
				 ...
				 * will be blitted to.
				 *
				 * @param sourceRectangle a clipping rectangle defining which
				 * part of this surface is to be blitted.
				 *
				...
				/**
				 * Displays the specified texture at the specified location on
				 * that surface, supposed to be the screen surface, at natural
				 * size (i.e. one pixel of the texture corresponds exactly to
				 ...
				 * @throw VideoException if the operation failed.
				 *
				 * @note If the texture is bigger than the surface, then only a
				 * part of the texture will be displayed.
				 *
				 ...
				 * @param y the ordinate screen coordinate.
				 *
				 * @param alpha a floating-point value in [0,1], from invisible
				 * to solid.
				 *
				 * @throw VideoException if the operation failed.
				 *
				 ...
				 * background based on specified alpha component.
				 *
				 * @param texture the texture to map.
				 *
				 ...
				 * horizontally, with a left and right border of width xOffset,
				 * scaling accordingly the texture.
				 *
				 * @param texture the texture to map.
				 *
				 * @param xOffset the first abscissa at which the texture should
				 * be horizontally displayed at.
				 *
				 * @param y the ordinate the texture will be displayed at.
				 *
				 * @throw VideoException if the operation failed.
				 *
				 * @note If the texture is bigger than the defined area, it will
				 ...
				 * supposed to be the screen surface, at natural size (i.e. one
				 * pixel of the texture corresponds exactly to one pixel on the
				 * screen).
				 *
				 * @param texture the texture to map.
				 *
				 ...
				 * supposed to be the screen surface, at natural size (i.e. one
				 * pixel of the texture corresponds exactly to one pixel on the
				 * screen), blended on the background based on specified alpha
				 * component.
				 *
				 ...
				 * supposed to be the screen surface, at natural size (i.e. one
				 * pixel of the texture corresponds exactly to one pixel on the
				 * screen), with a fade-in of specified duration.
				 *
				 * @param texture the texture to map.
				 *
				 * @param fadeInDuration the number of milliseconds the fade-in
				 * should last.
				 *
				 * @throw VideoException if the operation failed.
				 *
				 ...
				 * supposed to be the screen surface, at natural size (i.e. one
				 * pixel of the texture corresponds exactly to one pixel on the
				 * screen), with a fade-out of specified duration.
				 *
				 * @param texture the texture to map.
				 *
				 ...
				 * the full screen surface, possibly zooming (if the texture has
				 * a different size from the viewport) and distorting (if they
				 * have different width/height ratio), blended on the background
				 * based on specified alpha component, at specified onscreen
				 * location.
				 *
				 * @param texture the texture to map.
				 *
				 ...
				 * enough even for some realtime effects, if the CPU is fast or
				 * bitmaps are small.
				 *
				 * With antialiasing, the routines are typically used for
				 * pre-rendering content in higher quality (i.e. smoothing),
				 * hence a new render surface is created each time rotozoom is
				 * called.
				 *
				 */
				...
				/**
				 * Returns a newly created surface, obtained from this surface
				 * after a zoom of given factors.
				 *
				 * If, for this surface, pixel color is not 8 bit or 32 bit,
				 * then a 32-bit surface will be internally created and used for
				 * the zoom.
				 *
				 ...
				 * with it.
				 *
				 * The final rendering speed depends first on antialiasing being
				 * requested or not, then on resulting surface size.
				 *
				 * A surface is returned instead of zooming directly this
				 * surface to avoid accumulating rounding errors when zooming
				 * multiple times the same image: better use a constant source
				 * surface from which zoomed ones are created with various zoom
				 * factors than changing the same surface again and again.
				 *
				 * @param abscissaZoomFactor the zoom factor to be applied on
				 * the abscissa axis. If negative, the surface is flipped
				 * against this axis. No antialiasing will be performed in this
				 * case.
				 *
				 * @param ordinateZoomFactor the zoom factor to be applied on
				 * the ordinate axis. If negative, the surface is flipped
				 * against this axis. No antialiasing will be performed in this
				 * case.
				 *
				 * @param antialiasing if true and if zoom factors are positive,
				 * requests antialiasing (fine and expensive
				 * interpolation). 8-bit surfaces will never be antialiased.
				 *
				 */
				...
				/**
				 * Returns a newly created surface, obtained from this surface
				 * after a rotation of given angle and a zoom of given factor.
				 *
				 * If, for this surface, pixel color is not 8 bit or 32 bit,
				 * then a 32-bit surface will be internally created and used for
				 * the rotozoom.
				 ...
				 * avoid accumulating rounding errors when rotozooming multiple
				 * times the same image: better use a constant source surface
				 * from which rotozoomed ones are created with various angles
				 * and zoom factors than changing the same surface again and
				 * again.
				 *
				 * @param angle the angle of rotation, in degrees.
				 *
				 * @param zoomFactor the zoom factor to be applied on both
				 * surface dimensions. If negative, the surface is rotated of Pi
				 * before the angle is applied. No antialiasing will be
				 * performed in this case.
				 *
				 * @param antialiasing if true and if zoom factor is positive,
				 * requests antialiasing (fine and expensive
				 * interpolation). 8-bit surfaces will never be antialiased.
				 *
				 */
				...
				/**
				 * Returns a newly created surface, obtained from this surface
				 * after a rotation of given angle and zooms of given factors.
				 *
				 * If, for this surface, pixel color is not 8 bit or 32 bit,
				 * then a 32-bit surface will be internally created and used for
				 * the rotozoom.
				 *
				 * Ownership of the returned surface is transferred to the
				 * caller, who will have to deallocate it when finish with it.
				 *
				 * The final rendering speed depends first on antialiasing being
				 * requested or not, then on resulting surface size.
				 *
				 ...
				 * transferred to the caller, who therefore has to deallocate it
				 * when having finished with it.
				 *
				 * @throw VideoException if the operation failed or is not
				 * supported.
				 *
				...
				/**
				 * Returns this surface's content area, expressed thanks to an
				 * UprightRectangle.
				 *
				 * The content area is the smallest upright rectangle which
				 * encloses all the image pixels whose color is different from
				 * the guessed colorkey.
				 *
				 * @return A new UprightRectangle, whose ownership is
				 * transferred to the caller, who therefore has to deallocate it
				 * when having finished with it.
				...
				/**
				 * Loads an image, whose format will be auto-detected, from
				 * specified file, into this surface. This is a Surface factory.
				 *
				 ...
				 *
				 * @param blitOnly tells whether the loaded image surface should
				 * only be blitted into the supposed already existing internal
				 * surface and then be deallocated (if flag is true), or if this
				 * loaded surface should simply replace the former one (if flag
				 * is false).
				 *
				 * @param convertToDisplayFormat tells whether this loaded image
				 * should have its format converted to the screen's format, in
				 * order to offer faster blits if ever that surface was to be
				 * blitted multiple times to the screen (one-time-for-all
				 * conversion).
				 *
				 ...
				 *
				 * This method cannot be 'const' since this surface may have to
				 * be locked in order to access the pixels that are to be saved.
				 *
				 */
				 ...
				 * automatically set in an up-to-date state if necessary, so
				 * that it is fully redrawn, including its widgets.
				 *
				 * If it is a screen surface (either OpenGL or not), it will be
				 * updated in order to have the display match the surface
				 * buffer.
				 *
				 * Does nothing more if this surface is not a screen surface.
				 *
				 * @note When this method is called, the surface is supposed not
				 * to be locked.
				 *
				 ...
				 * video surface blit or lock will return
				 *
				 * - on hardware that does not support double-buffering, this is
				 * equivalent to calling the updateRectangle method with all
				 * arguments to zero
				 *
				 * @note The Surface::DoubleBuffered flag must have been passed
				 * to setMode, when setting the video mode, for this function to
				 * have a chance to perform hardware flipping.
				 *
				 * @see SDL_Flip
				 *
				 * @throw VideoException if the operation failed, including if
				 * this surface is not a screen surface.
				 *
				 */
				virtual void update() ;
				 ...
				 *
				 * Counts the number of times this surface has been updated once
				 * initialized.
				 *
				 * @note Even with a refresh rate of 100Hz, will not wrap around
				 * until more than one year.
				 *
				 * @note This corresponds only to full updates as performed by
				 * the update() method.
				 *
				 */
				 ...
				 * called.
				 *
				 * @note It is advised to call this method only once per frame,
				 ...
				 * avoid overdraw.
				 *
				 * @note STL too clumsy to understand list<const
				 * UprightRectangle const *> etc. (would require conversion by
				 * copy)
				 *
				 * @throw VideoException if the operation failed, including if
				 * this surface is not a screen surface.
				...
				/**
				 * Updates the part of this surface corresponding to specified
				 * rectangle.
				 *
				 ...
				 * boundaries (no clipping is done).
				 *
				 * @note If all arguments are equal to zero, the entire surface
				 * will be updated.
				 *
				 * @note The surface must not be locked when this method is
				 * called.
				 ...
				 *
				 * @note Meant to be overriden by widget class. Public since a
				 * widget has to be able to call it on its container.
				 *
				 */
				...
				/**
				 * Redraws this surface if needed, and handles automatically its
				 * tree of widgets so that it gets fully up-to-date and ready to
				 * be blitted.
				 *
				 * @return true if and only if the whole redraw was performed
				 * without any problem.
				 *
				 */
				virtual void redraw() ;
				...
				/**
				 * Registers specified widget to this surface, and put it at top
				 * level.
				 *
				 * @note The specified widget cannot be 'const' since, for
				 * example, a redraw event might result in a change in the
				 * widget.
				 *
				 */
				...
				/**
				 * Returns the surface that should be targeted by widgets having
				 * to blit their renderings to their container.
				 *
				 * Usually (but not always) the render target is the instance
				 * itself.
				 *
				 */
				 ...
				 * specified widget is put to front.
				 *
				 * @note Specified widget must already be registered to this
				 * surface.
				 *
				...
				/**
				 * Centers in the middle of the screen the position of the mouse
				 * cursor, by generating a mouse motion event.
				 *
				 * @note Must be called on a screen surface, with mouse enabled
				 * of course.
				 *
				 */
				...
				/**
				 * Sets the position of the mouse cursor, by generating a mouse
				 * motion event.
				 *
				 * @note Must be called on a screen surface, with mouse enabled
				 * of course.
				 ...
				 * it.
				 *
				 * When using a surface, in general there is no need to use this
				...
				/**
				 * Displays specified set of data, defined by dataArray, an
				 * array of dataCount IntegerData elements, as a curve drawn
				 * with pencilColor, on a background whose color is
				 * backgroundColor.
				 *
				 * The whole display will be scaled vertically to fit into
				 * inBox.
				 *
				 * On the contrary, the abscissa will be used as is.
				 *
				 * @note If inBox is a null pointer, then the curve will be
				 * drawn in the whole surface.
				 *
				 * @note When choosing the width of the container (inBox), to
				 * avoid truncating the graph, add 2*graphAbscissaOffset to the
				 * number of samples that describes the curve, and it will fit
				 * at best: both left and right boundaries will be taken into
				 * account.
				 *
				 */
				 ...
				 *
				 * @param filename the file from which the image will be loaded
				 *
				 * @param convertToDisplayFormat tells whether this loaded image
				 * should have its format converted to the screen's format, in
				 * order to offer faster blits if ever that surface was to be
				 ...
				 * converted surface should also have an alpha channel.
				 *
				 * @note The caller takes ownership of the created Surface, and
				 * must deallocate it when necessary.
				 *
				 ...
				 * VideoModule::setMode. Use VideoModule::interpretFlags
			...
			protected:
				 ...
				 *
				 * Nevertheless, only lock/unlock pair should be used by user
				 * programs.
				 *
				 * @see Ceylan::Lockable
				 *
				...
				/**
				 * Triggered when an abnormal inconsistency is detected between
				 * OSDL's surface state and its back-end counterpart.
				 *
				 * @note Those kind of errors, which are detected at runtime,
				 * mean that the code misbehaved badly, therefore an emergency
				 * stop might help.
				 *
				 */
				...
				/**
				 * This internal structure also contains private fields not used
				 * here.
				 *
				 */
				 ...
				 *
				 * - screen surfaces are not managed like backbuffers, since the
				 * formers have to update the screen
				 *
				 * - classical screen surfaces and OpenGL ones must be
				 * distinguished, since they update the screen differently (flip
				 * in double-buffering)
				 *
				 */
				...
				/**
				 * Tells whether this surface must be locked before modifying
				 * it.
				 *
				 */
				bool _mustBeLocked ;
				 ...
				 *
				 * @note Useful for the screen surface to compute the number of
				 * frames per second.
				 *
				 */
