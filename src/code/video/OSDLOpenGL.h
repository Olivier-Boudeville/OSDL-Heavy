...
namespace OSDL
{
	 ...
	 */
		 ...
		 * avoid adding a floating-point emulation library: it would require to
		 * eliminate many warnings (converting to 'GLLength' or 'GLCoordinate'
		 * from 'float').
		 *
		 */
...
namespace OSDL
{
	...
	namespace Video
	{
		...
		/**
		 * All OpenGL-specific operations and conventions are encapsulated here,
		 * be they related to 2D or 3D.
		 *
		...
		namespace OpenGL
		{
			...
			/**
			 * Mask used for OpenGL RGBA color specification for red, depending
			 * on the endianness of the local machine.
			 *
			 */
			 ...
			 *
			 *  - Reload: designates the same settings as the ones that were
			 * lastly used, just set them back in the state machine. This is
			 * useful for example when a resize occurs: the actual OpenGL
			 * context of the state machine is lost on some platforms, so the
			 * OSDL OpenGLContext object must set them again during the setMode
			 * call
			 *
			 * @note It results in loading the appropriate OpenGL settings for
			 * the selected mode, including projection, model view matrix, etc.
			 *
			 */
			...
			enum Feature {
				...
				/*
				 * If enabled, computed fragment color values will be blended
				...
				/*
				 * If enabled, culls polygons based on their winding in window
				 * coordinates.
				 *
				 */
				 ...
				 * lighting parameters will be used to compute the vertex color
				 * or index.
				 *
				 * Otherwise, simply associates the current color or index with
				 * each vertex.
				 *
				 */
			...
			/**
			 * An OpenGL context corresponds to the state of a running OpenGL
			 * screen.
			 *
			 * It can be managed on a per-application basis, or a per-window
			 * basis.
			 *
			 * OpenGL context objects have to store settings, so that they can
			 * be reloaded when required, see OpenGL::Reload flavour.
			 *
			...
			class OSDL_DLL OpenGLContext : public Ceylan::TextDisplayable
			{
				...
				public:
					 ...
					 *
					 * Smooth shading, the default, causes the computed colors
					 * of vertices to be interpolated as the primitive is
					 * rasterized, typically assigning different colors to each
					 * resulting pixel fragment. Also known as Gouraud shading.
					 *
					 * In either case, the computed color of a vertex is the
					 * result of lighting, if lighting is enabled, or it is the
					 * current color at the time the vertex was specified, if
					...
					/**
					 * Describes which facets should be culled, i.e.  specifies
					 * whether front-facing or back-facing facets are culled,
					 * when facet culling is enabled.
					 *
					 * Facets include triangles, quadrilaterals, polygons, and
					 * rectangles.
					 *
					 * @note If FrontAndBack is selected, then no facets are
					 * drawn, but other primitives such as points and lines are.
					 *
					 * Front and back are defined according to the normal vector
					 * of a facet, which can be determined according to the dot
					 * product of ordered listed vertices.
					 *
					 ...
					 * to have clockwise winding if an imaginary object
					 * following the path from its first vertex, its second
					 * vertex, and so on, to its last vertex, and finally back
					 * to its first vertex, moves in a clockwise direction about
					 * the interior of the polygon.
					 *
					 * On the contrary, the polygon's winding is said to be
					 * counterclockwise if the imaginary object following the
					 * same path moves in a counterclockwise direction about the
					...
					/**
					 * Creates a new blank OpenGL context, according to the
					 * specified flavour and color depth.
					 *
					 ...
					 * Video renderers, since they may not be used, and OpenGL
					 * must be specified during the setMode phase.
					 *
					 * @see OpenGLFlavour
					 *
					 * @throw OpenGLException if the OpenGL state machine
					...
					/**
					 * Sets the OpenGL 2D flavour, i.e. the settings deemed the
					 * most appropriate for 2D rendering.
					 *
					...
					/**
					 * Sets the OpenGL 3D flavour, i.e. the settings deemed the
					 * most appropriate for 3D rendering.
					 *
					 * @throw OpenGLException if the operation failed.
					 *
					 * @note Calling selectFlavour should be preferred, as it
					...
					/**
					 * Blanks the current settings in the actual OpenGL state
					 * machine.
					 *
					 ...
					 * OpenGL context under various circumstances, including
					 * resizing.
					 *
					 * @see OpenGL::Reload
					 *
					 ...
					 *
					 * @param sourceFactor specifies which of the nine methods
					 * is used to scale the source color components.
					 *
					 ...
					 *
					 * @param newShadingModel the new shading technique that
					 * shall be used.
					 *
					 * @throw OpenGLException should an error occur.
					 *
					 ...
					 *
					 * @param culledFacet tells whether front, or back, or front
					 * and back facets should be culled.
					 *
					 * @param frontOrientation defines what are front facing
					 * polygons, depending on their winding.
					 *
					 * @param autoEnable will specifically enable the use of
					 * culling iff true (it will not be only set).
					 *
					 * @throw OpenGLException should an error occur.
					 *
					 */
					 ...
					 *
					 * @param newStatus if true, will enable the depth buffer,
					 ...
					 * triggered accordingly, to avoid distorted graphics: it
					 * ensures that the viewport aspect ratio is the same as the
					 * one of the viewing volume.
					 *
					 * @param width the width of the viewport.
					 *
					 * @param height the height of the viewport.
					 *
					 * @param lowerLeftCorner the coordinates of the lower left
					 * corner of the viewport rectangle in pixels. The default
					 * is the origin, (0,0).
					 *
					 * @throw OpenGLException if either width or height is
					 * negative, or if this method is called between the
					 ...
					 * volume is a box, whose sides are parallel to the main
					 * axes.
					 *
					 * The viewer is located at the origin, looking towards the
					 * negative z values, his up vector being (0;1;0).
					 *
					 ...
					 * graphics
					 *
					 *  - the box is cut in two equal parts by the axes x=0 and
					 * y=0
					 *
					 *  - the box will lie between the two planes z=-near and
					 * z=-far
					 *
					 * Therefore, for a box width of w, the box will range
					 * between -w/2 and w/2 on the x axis, and between -w*r/2
					 * and w*r/2 on the y axis, where r is the viewport aspect
					 * ratio: r = (viewport height) / (viewport width).
					 *
					 * @param width the length of the box edge which is parallel
					 * to the x axis.
					 *
					 ...
					 *
					 * @throw OpenGLException if this method is called between
					...
					/**
					 * Uploads specified surface as an OpenGL texture in OpenGL
					 * context.
					 *
					 * @param source the surface that must be converted into
					 * texture. Its width and height must be a power of 2.
					 *
					 * It is no longer needed once the texture is loaded, hence
					 * it can be deallocated by the caller.
					 *
					 * @throw OpenGLException if the operation failed.
					 *
					 *
					...
					*/
					 ...
					 * Static section.
					 *
					 * All static state-changing methods should probably set as
					 * member methods of a context instance, even if it would
					 * force the caller of always retrieving the context.
					 *
					 */
					 ...
					 * (if true) or safest ones (if false), which are useful if
					 * the best just failed to be set properly.
					 *
					 * @note The corresponding attributes will not be taken into
					 * account until the next call to VideoModule::setMode.
					 *
					 * @throw OpenGLException if the operation failed.
					 *
					 */
					 ...
					 * before a VideoModule::setMode call (otherwise they will
					 * not be taken into account until the next setMode call),
					 * and no OpenGL context may already exist.
					 *
					 */
					 ...
					 * VideoModule::setMode.
					 *
					 * @param newStatus, double buffering will be enabled iff
					 * true (and available).
					 *
					 * @throw OpenGLException should an error occur.
					 *
					 ...
					 * VideoModule::setMode.
					 *
					 * @note Does not enable the depth buffer, see
					 * setDepthBufferStatus.
					 *
					...
					/**
					 * Returns whether the OpenGL fullscreen antialiasing (FSAA)
					 * is available.
					 *
					 * @note Requires the GL_ARB_MULTISAMPLE extension.
					 *
					 * @return 0 if the feature is not available, otherwise the
					 * non-null number of samples used (ex: 4 means 4x
					 * antialiasing).
					 *
					 * @throw OpenGLException should an error occur.
					 *
					...
					/**
					 * Sets the OpenGL fullscreen antialiasing (FSAA) status
					 * (enabled or disabled).
					 *
					 * @note Will not take effect until the next call to
					 * VideoModule::setMode.
					 ...
					 *
					 * @param samplesPerPixelNumber number of samples per pixel
					 * when multisampling (FSAA) is enabled. Is usually 2, 4,
					 * 16, etc. This parameter is ignored if newStatus is false.
					 *
					 * @throw OpenGLException should an error occur.
					 *
					 */
					 ...
					 * available.
					 *
					 * @return true iff the hardware acceleration is available.
					 *
					 * @throw OpenGLException should an error occur.
					 *
					 */
					...
					/**
					 * Sets the OpenGL hardware-acceleration status (enabled or
					 * disabled).
					 *
					 * @note Will not take effect until the next call to
					 * VideoModule::setMode.
					 ...
					 *
					 * @param samplesPerPixelNumber number of samples per pixel
					 * when multisampling (FSAA) is enabled.
					 *
					 * @throw OpenGLException should an error occur.
					 *
					 */
					...
					/**
					 * Returns whether the OpenGL synchronization with vertical
					 * blank retrace (VSYNC) is activated.
					 *
					 ...
					 * VideoModule::setMode.
					 *
					 * @param newStatus vsync synchronization will be enabled
					 ...
					 * platform, i.e. the setting reported a success and reading
					 * back the value confirmed it; if it failed, no exception
					 * is thrown, only false is returned.
					 *
					 */
					 ...
					 *
					 * @param redSize the variable where the size in bits of the
					 * red component will be written.
					 *
					 * @param greenSize the variable where the size in bits of
					 * the green component will be written.
					 *
					 * @param blueSize the variable where the size in bits of
					 * the blue component will be written.
					 *
					 * @param alphaSize the variable where the size in bits of
					 * the alpha component will be written.
					 *
					 ...
					 * VideoModule::setMode.
					 *
					 * @param plannedBpp the planned color depth, in bits per
					 * pixel.
					 *
					 * @throw OpenGLException if the operation failed.
					 *
					 ...
					 * VideoModule::setMode.
					 *
					 * @param redSize the size of the red component, in bits per
					 * pixel.
					 *
					...
					/**
					 * Returns a textual description of the availability of the
					 * OpenGL features.
					 *
					 * @note The description is only useful after a
					 * VideoModule::setMode call; before it may even crash the
					 * program.
					 *
					 * @throw OpenGLException if the operation failed.
					 *
					...
					/**
					 * Returns an user-friendly description of the specified
					 * OpenGL flavour.
					 *
					...
					/**
					 * Tells whether the OpenGL context is lost (and therefore
					 * must be reloaded) on resize.
					 *
					 * @see OpenGL::Flavour, for Reload.
					 *
					 */
					...
					/**
					 * Tells whether the OpenGL context is lost (and therefore
					 * must be reloaded) when switching application.
					 *
					 * @see OpenGL::Flavour, for Reload.
					 *
					 */
					...
					/**
					 * Tells whether the OpenGL context is lost (and therefore
					 * must be reloaded) when changing the color depth.
					 *
					 * @see OpenGL::Flavour, for Reload.
					 *
					 */
			...
			protected:
					...
					/**
					 * Updates the current projection so that its aspect ratio
					 * matches the one of the viewport.
					 *
					 * @throw OpenGLException if the current projection mode
					 * cannot be handled.
					 *
					 */
					 ...
					 * supported.
					 *
					 * If not (i.e. if false is returned), then calling the
					 * GetGLAttribute function would throw an exception.
					 *
					...
					/**
					 * Width of the viewport, useful to ensure aspect ratio is
					 * well managed.
					 *
					 */
					...
					/**
					 * Height of the viewport, useful to ensure aspect ratio is
					 * well managed.
					 *
					 */
					...
					/**
					 * Width of the projection volume, so that projection can be
					 * automatically updated when the viewport area changes.
					 *
					 */
			 ...
			 *
			 * Some of them are defined here just to use GLU conventions (ex:
			 * GLUint) without needing to include GLU headers, that may or may
			 * not be available.
			 *
			 * This is a form of code duplication, but GLU conventions are not
			 * expected to change often, and the other solutions are worse:
			 * hiding actual datatypes behind indirection pointers (would need a
			 * lot of code for that and would decrease runtime performance) or
			 * including GLU headers (would require to include as well
			 * OSDLConfig.h in OSDL headers, and then would force to install
			 * OSDLConfig.h, which is not wished).
			 *
			 */
			namespace GLU
			{
