...
namespace OSDL
{
	...
	namespace Video
	{
		...
		/**
		 * An overlay is an image, encoded in a specific format, that can be
		 * blitted onto a Surface.
		 *
		 ...
		 *  - V=Normalised RY
		 *
		 * This is a colour model that describes colour information in terms of
		 * luminance (Y) and chrominance (U, V).
		 *
		 * YUV can be seen as transformed RGB values such that there is low
		 * correlation between the three coordinates.
		 *
		 * This is the colour space representation used in PAL video and
		 * television transmission.
		 *
		 * @note The term 'overlay' is a misnomer since, unless the overlay is
		...
		class OSDL_DLL Overlay : public Ceylan::Lockable
		{
			...
			public:
				...
				/**
				 * Creates a YUV overlay of the specified width, height and
				 * format, linked with the current screen surface.
				 *
				 ...
				 * of this overlay.
				 *
				 * @throw OverlayException if video module could not be
				 * retrieved or if the overlay creation failed.
				 *
				 * @see EncodingFormat
				 *
				 */
				...
				/**
				 * Blits the overlay to the display, at the specified location,
				 * at its original size.
				 *
				 * @param x abscissa of the destination surface where this
				 * overlay will be blitted.
				 *
				 ...
				 * it.
				 *
				 * When using a overlay, in general there is no need to use this
			...
			protected:
				 ...
				 *
				 * Nevertheless only lock/unlock pair should be called by user
				 * programs.
				 *
				 * @see Ceylan::Lockable
				 *
