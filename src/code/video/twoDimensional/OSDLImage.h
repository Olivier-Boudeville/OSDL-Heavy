...
namespace OSDL
{
	...
	namespace Video
	{
		...
		namespace TwoDimensional
		{
			...
			/**
			 * Provides an encapsulation for any picture instance that can be
			 * loaded as resource.
			 *
			 ...
			 *	- XPM
			 *
			 * Our belief is however that the fewer formats you use, the simpler
			 * it will be for everyone.
			 *
			 * We therefore recommend using two of them, which moreover are
			 * patent-less standards and should cover most if not all needs:
			 *
			 * - JPG: to compress, with little loss, full-blown images such as
			 * photographies, with high color depth
			 *
			 * - PNG: to compress without loss computer-generated images, with
				...
				public Ceylan::LoadableWithContent<Surface>
			{
				...
				public:
					...
					/**
					 * This Image constructor uses given filename to load the
					 * picture from the corresponding file.
					 *
					 * Actual picture format is guessed, based on the file
					 * content (and not on the filename).
					 *
					 * @param imageFilename the name of the file containing the
					 * targeted image, whose format (PNG, JPEG, etc.) will be
					 ...
					 * XPM, LBM, PCX, GIF, JPEG, PNG, and TIFF formats. JPEG
					 * and PNG are the two strongly recommended formats.
					 *
					 ...
					 * surface, while updating specified mask so that it selects
					 * only icon pixels that should remain visible.
					 *
					 * @note Ownership of the surface and the mask is
					 * transferred to the caller, which has to deallocate them
					 * when necessary.
					 *
					 * @note The icon must have a palette, and its size should
					 * be 32 * 32 pixels.
					 *
					 ...
					 * specified file into target surface.
					 *
					 ...
					 * internal surface and then be deallocated (if flag is
					 * true, notably useful to load an image into the screen
					 * surface), or if this loaded surface should simply replace
					 * the former one (if flag is false, usual case for shadow
					 * surfaces).
					 *
					 * @param convertToDisplay tells whether this loaded image
					 * should have its pixel format converted to the screen's
					 * format, in order to allow for faster blits if ever that
					 ...
					 * whether the converted surface should also have an alpha
					 * channel.
					 *
					 ...
					 * surface as target surface, since this surface is special
					 * and should not be deallocated as the others may be.
					 *
					 */
					 ...
					 * surface.
					 *
					 * @param targetSurface the surface that should contain the
					 * loaded image.
					 *
					...
					/**
					 * Saves specified surface on file, with specified filename,
					 * in PNG format.
					 *
					 * @param targetSurface the surface that should be saved on
					 * file.
					 *
					 * This parameter cannot be 'const' since the surface may
					 * have to be locked to read its pixels, so that they can be
					 * stored in the image file.
					 *
					 * @param filename the name of the PNG file.
					 *
					 * @param overwrite tells whether any already existing file
					 * should be overwritten.
					 *
					 * The default is true, if false an exception will be raised
					 * should a corresponding file be found.
					 *
					 ...
					 * @param filename the name of the BMP file.
					 *
					 * @param overwrite tells whether an already existing file
					 * should be overwritten.
					 *
					 * The default is true, if false an exception will be raised
					 * should a corresponding file be found.
				...
				protected:
					...
					/**
					 * Tells whether when loaded this image should be converted
					 * to screen format.
					 *
					 */
					...
					/**
					 * Tells whether when loaded this image should have an alpha
					 * channel when converted to screen format.
					 *
			...
			typedef struct
			{
				...
				/*
				 * g++ (gcc) needs this __attribute__, but Visual C++ does not
				 * understand it.
				 *
