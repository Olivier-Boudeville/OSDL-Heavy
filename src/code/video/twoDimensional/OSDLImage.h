...
namespace OSDL
{
	...
	namespace Video
	{
		...
		namespace TwoDimensional
		{
			 ...
			 *
			 * The Image interface is mostly a set of related functions (static methods), 
			 * including factories. 
			 * 
			 ...
			 *		- XPM
			 *
			 * Our belief is however that the fewer formats you use, the simpler it will be for
			 * everyone. 
			 *
			 * We therefore recommend using two of them, which moreover are patent-less
			 * standards and should cover most if not all needs :
			 *		- JPG : to compress, with little loss, full-blown images such as 
			 * photographies, with high color depth
			 *		- PNG : to compress without loss computer-generated images, with or  
			...
			class Image : public Ceylan::TextDisplayable
			{
				...
				public:
					...
					/**
					 * This Image constructor uses given filename to load the picture from the
					 * corresponding file .
					 *
					 */
					explicit Image( const std::string & filename ) throw( ImageException ) ;
					...
					/**
					 * Loads specified icon, and returns the corresponding surface, while updating
					 * specified mask so that it selects only icon pixels that should remain
					 * visible.
					 * 
					 * @note Ownership of the surface and the mask is transferred to the caller,
					 * which has to deallocate them when necessary.
					 *
					 * @note The icon must have a palette, and its size should be 32 * 32 pixels.
					 *
					...
					/**
					 * Loads an image, whose format will be auto-detected, from specified file 
					 * into target surface.
					 *
					 *
					 * @param targetSurface the surface that should contain eventually the loaded
					 * image.
					 *
					 * @param filename the name of the image file.
					 *
					 * @param blitOnly tells whether the loaded image surface should only be 
					 * blitted into the supposed already existing internal surface and then be
					 * deallocated (if flag is true, notably useful to load an image into the 
					 * screen surface), or if this loaded surface should simply replace the former
					 * one (if flag is false, usual case for shadow surfaces).  
					 * 
					 * @param convertToDisplay tells whether this loaded image should
					 * have its pixel format converted to the screen's format, in order to 
					 * allow for faster blits if ever that surface was to be blitted 
					 * multiple times to the screen (one-time-for-all conversion). <b>Beware</b> :
					 * setMode must have been called before convertToDisplay is chosen.
					 *
					 * @param convertWithAlpha if the conversion to screen format is
					 * selected (convertToDisplay is true), tells whether the 
					 * converted surface should also have an alpha channel. 				 
					 *
					 * @note The auto-detected format must be of course a supported image format.
					 *
					 * @note Specifying a blitOnly option set to true is especially convenient 
					 * when specifying the screen's surface as target surface, since this surface 
					 * is special and should not be deallocated as the others may be.
					 * 
					 *
					 */						
					static void Load( Surface & targetSurface, 
						...
						bool convertWithAlpha = true  ) throw( ImageException ) ;  	
					 ...
					 * multiple times to the screen (one-time-for-all conversion).
					 *
					 * @param convertWithAlpha if the conversion to screen format is
					 * selected (convertToDisplay is true), tells whether the 
					 * converted surface should also have an alpha channel. 				 
					 ...
					 *
					 * @param targetSurface the surface that should be saved on file. This 
					 * parameter cannot be 'const' since the surface may have to be locked to read
					 * its pixels, so that they can be stored in the image file.
					 *
					 * @param filename the name of the PNG file.
					 *
					 * @param overwrite tells whether an already existing file should be 
					 * overwritten. The default is true, if false an exception will be raised
					 * should a file be found.
					 *
					 * @note This method is especially convenient for screenshots.				 
					 *
