...
#include "OSDLFromGfx.h"        // for graphic primitives not exported by SDL_gfx
	...
	ColorElement red, ColorElement green, ColorElement blue, ColorElement alpha ) throw()
{
	...
	return ( ::hlineColor( & targetSurface.getSDLSurface(), xStart, xStop, y, 
		((Uint32) red << 24) | ((Uint32) green << 16) 
			| ((Uint32) blue << 8) | ((Uint32) alpha) ) == 0 ) ;
...
{
	return ( ::hlineColorStore( & targetSurface.getSDLSurface(), xStart, xStop, y, 
		actualColor) == 0 ) ;
}
...
bool Line::drawVertical( Surface & targetSurface, Coordinate x, 
	...
	ColorElement red, ColorElement blue, ColorElement green, ColorElement alpha ) throw()
{
	...
	ColorElement red, ColorElement green, ColorElement blue, ColorElement alpha ) throw()
{
	...
	{
		return ( ::aalineColorInt( & targetSurface.getSDLSurface(), xStart, yStart, xStop, yStop,
			((Uint32) red << 24) | ((Uint32) green << 16) | ((Uint32) blue << 8) 
			| ((Uint32) alpha), VideoModule::GetEndPointDrawState() ) == 0 ) ;		
	}
	else
	{
		// Warning : VideoModule::GetEndPointDrawState() cannot be taken into account :
		
		return ( ::lineColor( & targetSurface.getSDLSurface(), xStart, yStart, xStop, yStop,
			((Uint32) red << 24) | ((Uint32) green << 16) | ((Uint32) blue << 8) 
			| ((Uint32) alpha) ) == 0 ) ;		
	}
...
bool Line::draw( Surface & targetSurface, Point2D & firstPoint, Point2D & secondPoint,
	...
	Pixels::ColorDefinition colorDef, Length squareEdge ) throw() 
{
	...
	return drawCross( targetSurface, center.getX(), center.getY(), colorDef, squareEdge ) ;
		 
}	


bool Line::drawCross( Surface & targetSurface, Coordinate xCenter, Coordinate yCenter,
	Pixels::ColorDefinition colorDef, Length squareEdge ) throw() 
{

	// Draws '\' :
	bool result = draw( targetSurface, 
		static_cast<Coordinate>( xCenter - ( (FloatingPointCoordinate) squareEdge / 2 ) ),
		static_cast<Coordinate>( yCenter - ( (FloatingPointCoordinate) squareEdge / 2 ) ),
		static_cast<Coordinate>( xCenter + ( (FloatingPointCoordinate) squareEdge / 2 ) ),
		static_cast<Coordinate>( yCenter + ( (FloatingPointCoordinate) squareEdge / 2 ) ),
		colorDef ) ;
