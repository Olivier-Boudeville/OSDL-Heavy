...
#include "SDL_ttf.h"          // for font primitives (no TTF_Font forward declaration possible)
	...
	namespace Video
	{
		...
		namespace TwoDimensional
		{
			...
			{	
				 ...
				 * Truetype font.
				 *
				 * A font can contain faces, which are specific sub-fonts, due to their size and
				 * style.
				 *
				 ...
				 * 127. Latin-1 continues and adds more common international symbols to define
				 * through character 255.
				 *
				 * Plain ASCII is UTF-8 compatible.
				 *
				 * The principle of these rendering methods is to return a surface with the chosen
				 * glyph, word or full text drawn with the specified color, with no visible
				 * background : thanks to color key or alpha-blending, only the text can be seen 
				 * so that this returned surface can be directly blitted onto any already existing
				 * background that will be hidden only by the pixels corresponding to the text.
				 * 
				 ...
				 * be negative.
				 *
				 * @note There exist some Truetype fonts that are actually 'fixed width'. They are 
				 * supported by this class, even if the special case of fixed font could be managed
				 * thanks to the FixedFont class too. 
				 *
				 */	
				class TrueTypeFont : public Font
				{
					...
					public:
						 ...
						 * usually a .ttf, .TTF or a .FON file. The filename will be interpreted
						 ...
						 * variable, if any.
						 *
						 * @param pointSize the size of a point, based on 72 dots per inch. This 
						 ...
						 * should have already been converted to display.
						 *
						 ...
						 *
						 * @throw TextException if the file could not be found or if the font 
						 * library did not initialize properly.
						 *
						 *
						 */
						TrueTypeFont( 
							...
							RenderCache cacheSettings = GlyphCached ) throw( TextException ) ;
						 ...
						 *
						 * @note Combining Font::Underline with anything can cause a segfault, 
						 * other combinations may also do this.
						 *
						 * @note Changing the font style flush the internal cache of previously
						 * rendered glyphs.
						 *
						 * @param newStyle a bitmask of the desired rendering styles. 
						 *
						 * Ex : myFont.setRenderingStyle( Font::Italic | Font:Bold ) ;
						 *
						 ...
						 * (actually, the origin in the glyph referential) and the same part of the
						 * next glyph. 
						 * It includes therefore the width of this glyph and the space between the
						 * next glyph that could be rendered afterwards. 
						 *
						 ...
						 *
						 * @note For the moment, no specific use of this information can be done
						 * through the library.
						 *
						 */
						 ...
						 *
						 * The metrics are described as an upright rectangle which contains the
						 * glyph, defined from its upper left corner (xMin, yMax) to its lower
						 ...
						 *
						 * The caller gets ownership of this returned rectangle, it has therefore
						 * to delete it when the rectange is not to be used any more.
						 *
						 * @throw TextException on error, for example if the specified character
						 * does not exist in this font.
						 *
						 * Width is xMax - xMin.
						 * Height is yMax - yMin.
						 * BearingX is xMin
						 ...
						 *
						 */
						virtual UprightRectangle & getBoundingBoxFor( Ceylan::Unicode glyph,
							 SignedLength & advance ) const throw( TextException ) ;
							
						 
						/**
						 * Returns a rectangular bounding box corresponding to the rendering of
						 * specified text, encoded in Latin-1. 
						 *
						 * Correct kerning is done to get the actual width. The height returned 
						 * is the same as you can get using the getHeight method.
						 *
						 * The upright rectangle has its lower left corner set to the origin. 
						 *
						 * @param text the Latin-1 encoded string to size up.
						 *
						 * @return an upright rectangle enclosing the text (bounding box), whose
						 * lower left corner is located at the origin.
						 *
						 * @throw TextException if an error occured, for example if a requested
						 * glyph does not exist.
						 *
						 */ 
						virtual UprightRectangle & getBoundingBoxFor( const std::string & text )
							const throw( TextException ) ;
							 
							 
						/**
						 * Returns a rectangular bounding box corresponding to the rendering of
						 * specified text, encoded in UTF-8. 
						 *
						 * Correct kerning is done to get the actual width. The height returned 
						 * is the same as you can get using the getHeight method.
						 *
						 ...
						 *
						 */ 
						virtual UprightRectangle & getBoundingBoxForUTF8( const std::string & text )
							const throw( TextException ) ;
						...
						/**
						 * Returns a rectangular bounding box corresponding to the rendering of
						 * specified text, encoded in Unicode. 
						 *
						 * Correct kerning is done to get the actual width. The height returned 
						 * is the same as you can get using the getHeight method.
						 *
						 * The upright rectangle has its lower left corner set to the origin. 
						 *
						 * @param text the Unicode encoded null terminated string to size up.
						 *
						 * @return an upright rectangle enclosing the text (bounding box), whose
						 * lower left corner is located at the origin.
						 *
						 ...
						 *
						 * @param quality the chosen rendering quality. If 'Shaded', the current
						...
						/**
						 * Renders specified glyph, specified as a Unicode-encoded character, in
						 * specified color, in a returned surface. 
						 *
						 * Unicode management is available with Truetype fonts, but may be not for
						 ...
						 * background color will be used.
						 *
						 * @param textColor the color definition for the glyph. This becomes first 
						 * index of colormap, in Solid and Shaded qualities.
						 *
						 * @return a newly allocated Surface, whose ownsership is transferred to 
						 * the caller.
						 *
						 * @throw TextException on error, for example if the specified glyph was not
						 * found.
						 *
						 */
						virtual Surface & renderUnicodeGlyph( Ceylan::Unicode character, 
								RenderQuality quality = Solid,
						 ...
						 * returned surface. The caller is responsible for deleting any returned
						 * surface.
						 *
						 * The caller is responsible for deleting the returned surface.
						 *
						 ...
						 * background color will be used.
						 *
						 * @param textColor the color definition for the text. This becomes first 
						 * index of colormap, in Solid and Shaded qualities.
						 *
						 * @return a newly allocated Surface, whose ownsership is transferred to 
						 * the caller.
						...
						/**
						 * Renders specified text, encoded in UTF-8, in specified color, in a 
						 * returned surface. The caller is responsible for deleting any returned
						 * surface.
						 *
						 * The caller is responsible for deleting the returned surface.
						 ...
						 *
						 */
						virtual Surface & renderUTF8Text( const std::string & text, 
								RenderQuality quality = Solid,
								Pixels::ColorDefinition textColor = Pixels::White ) 
							throw( TextException ) ;
						...
						/**
						 * Renders specified text, encoded in Unicode, in specified color, in a 
						 * returned surface. The caller is responsible for deleting any returned
						 * surface.
						 *
						 * The caller is responsible for deleting the returned surface.
						 ...
						 *
						 */
						virtual Surface & renderUnicodeText( const Ceylan::Unicode * text, 
								RenderQuality quality = Solid,
						...
						/**
						 * Determines whether Unicode data is to be swapped relative to the CPU
						 * endianess.
						 *
						 * @param newStatus if true, then Unicode data is byte swapped relative to
						 * the CPU's native endianess. If false, do not swap Unicode data, use the
						 * CPU's native endianess.
						 *
						 * A Unicode_BOM_NATIVE or Unicode_BOM_SWAPPED character in a string will
						 * temporarily override this setting for the remainder of that string,
						...
						/**
						 * Returns the last error that occured relatively to Truetype fonts.
						 *
						 *
						 */	
					...
					protected:
						...
						/**
						 * Renders specified glyph (Unicode character) in specified color, on a 
						 * new surface, directly thanks to the font backend : no cache is taken 
						 * into account. 
						 *
						 ...
						 *
						 */
						virtual Surface & basicRenderUnicodeGlyph( Ceylan::Unicode character,
								RenderQuality quality, Pixels::ColorDefinition glyphColor )
							throw( TextException ) ;
						...
						/**
						 * Records how many fonts are currently on use, to perform clean-up as 
						 * soon as no one exists.
						 *
						 */
