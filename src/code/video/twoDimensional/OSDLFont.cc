...
Ceylan::System::FileLocator Text::Font::FontFileLocator( FontPathEnvironmentVariable ) ;
	...
	_alineaWidth( 0 )
{
	...
	/*
	 * _spaceWidth and _alineaWidth cannot be initialized in this Font mother class since getWidth
	 * is abstract for it (pure virtual). Therefore each child class should initialize it.
	 *
	 */
	...
	switch( _cacheSettings )
	{
		...
		/*
		 * The several sub-switches allow to get rid of following parameter in signature :
		 * 'Ceylan::SmartResourceManager<CharColorQualityKey>::CachePolicy cachePolicy =
		 *    Ceylan::SmartResourceManager<CharColorQualityKey>::DropLessRequestedFirst'
		 * which would not be compliant with a SmartResourceManager<StringColorQualityKey>, 
		 * which is also needed. 
		 *
		 * The abstract AllowedCachePolicy is therefore to be converted to its templated version.
		 *
		 */
		...
		case GlyphCached:
			...
			SmartResourceManager<CharColorQualityKey>::CachePolicy actualGlyphPolicy ;
			// Let's convert our allowed cache policy into a templated-ready cache policy :
			switch( cachePolicy )
			{	
				case NeverDrop:
					actualGlyphPolicy = SmartResourceManager<CharColorQualityKey>::NeverDrop ;
					break ;
				...
				default:
					Ceylan::emergencyShutdown( "OSDL::Video::TwoDimensional::Font constructor "
		...
		case WordCached:
			...
			SmartResourceManager<StringColorQualityKey>::CachePolicy actualWordPolicy ;
			// Let's convert our allowed cache policy into a templated-ready cache policy :
			switch( cachePolicy )
			{	
				case NeverDrop:
					actualWordPolicy = SmartResourceManager<StringColorQualityKey>::NeverDrop ;
					break ;
			...
			_textCache = new SmartResourceManager<StringColorQualityKey>( quota, 
				actualWordPolicy ) ;
			break ;
		...
		case TextCached:
			...
			SmartResourceManager<StringColorQualityKey>::CachePolicy actualTextPolicy ;
			// Let's convert our allowed cache policy into a templated-ready cache policy :
			switch( cachePolicy )
			{	
				case NeverDrop:
					actualTextPolicy = SmartResourceManager<StringColorQualityKey>::NeverDrop ;
					break ;
...
void Font::setBackgroundColor( Pixels::ColorDefinition newBackgroundColor ) throw()
{
...
std::string Font::describeGlyphFor( Ceylan::Latin1Char character ) const throw()
{
	...
	res.push_back( "Advance is " + Ceylan::toString( getAdvance( character ) ) ) ;
	
	res.push_back( "Width is " + Ceylan::toString( getWidth( character ) ) ) ;
	
	res.push_back( "Width offset is " + Ceylan::toString( getWidthOffset( character ) ) ) ;
	...
	return "Informations about the glyph corresponding to the character '" 
		+ Ceylan::toString( character ) + "' : " + Ceylan::formatStringList( res ) ;
...
Width Font::getInterGlyphWidth() const throw()
{
	...
	// Scales with the lettre width, minimum is one :
...
OSDL::Video::Surface & Font::renderLatin1Text( const std::string & text,
	RenderQuality quality, Pixels::ColorDefinition textColor ) throw( TextException )
{
	...
	switch( _cacheSettings )
	{
		...
		case GlyphCached:
			/*
			 * Nothing else to do than full rendering (fire and forget), glyph that are cached or
			 * not are managed directly at the glyph level.
			 *
			 */
		...
		default:
			Ceylan::emergencyShutdown( "OSDL::Video::TwoDimensional::Font::renderLatin1Text : "
				"unexpected cache settings" ) ;
			break ;
...
void Font::blitLatin1Text( Surface & targetSurface, Coordinate x, Coordinate y, 
		const std::string & text, RenderQuality quality, Pixels::ColorDefinition textColor ) 
	throw( TextException ) 			
{
	 	
	/*
	 * Shortcut :
	 * Have a chance to retrieve and blit from the text cache instead of creating, blitting and
	 * destroying a Surface just for this blit (if the text cache is used; word cache has far
	 * too few chances of having 'text', it is not interrogated here as a shortcut) :
	 *
	 */ 
	if ( _cacheSettings == TextCached )
	{
		...
		if ( res != 0 )
		{
			...
			dynamic_cast<const Surface *>( res )->blitTo( targetSurface, x, y ) ;
	...
	// Here we have to ask a full one-shot rendered surface (possibly with cache help) :
	Surface & res = renderLatin1Text( text, quality, textColor ) ;
	
	// Useful for testing : res.savePNG( Ceylan::toString( y ) + ".png" ) ;
	...
	catch( const VideoException & e )
	{
		...
		throw TextException( "Font::blitLatin1Text : blit failed : " + e.toString() ) ;
	}	
...
OSDL::Video::Surface & Font::renderLatin1MultiLineText( Length width, Length height, 
	const std::string & text, TextIndex & renderIndex, Coordinate & lastOrdinateUsed,
		...
		throw( TextException )
{
	...
	Surface & res = * new Surface( Surface::Hardware | Surface::ColorkeyBlit, 
		width, height, /* bpp */ 32, redMask, greenMask, blueMask, /* no alpha wanted */ 0 ) ;

	/*
	 * Avoid messing text color with color key.
	 * 'Pixels::selectColorDifferentFrom' could be used but here a full surface fill can be saved
	 * in most cases (as long as the text color is not pure black).
	 *
	 */
	...
	// This integer division returns the maximum usable number of lines in this box :
	...
	#ifdef OSDL_DEBUG_FONT
	LogPlug::debug( "Font::renderLatin1MultiLineText : " + Ceylan::toString( maxLines )
		+ " line(s) available to render following text : '" + text + "'." ) ;
	#endif
	
	/*
	 * Auto-centering vertically the usable area for text in its surface should not be enforced here
	 * since it would hinder placing the returned surface exactly where appropriate in its 
	 * container.
	 * Hence disabled : 'Height lineHeight = ( height - maxLines * lineSkip ) / 2 ;'
	 *
	 */
	Height lineHeight = 0 ; 
	...
	/*
	 * Having a justified text implies that all words in a line of text must be rendered before
	 * starting the blits for this line. 
	 *
	 * If word or text caches are used, then word renderings are directly taken from them : it 
	 * prevents from creating, rendering and destroying uselessly as many surfaces as words.
	 *
	 * If no word or text cache is used, these renderings have to be kept nonetheless, whether only
	 * a glyph cache is used or not. Therefore a word cache is temporarily used to speed up this 
	 * two-pass line rendering. Glyph cache, if any, is used as well nonetheless and will be fed,
	 * whereas word cache will be deallocated after this rendering. 
	 *
	 */
	
	bool createTemporaryWordCache = ( _cacheSettings == GlyphCached || _cacheSettings == None ) ;
	...
	if ( createTemporaryWordCache )
	{
		...
		if ( _textCache != 0 )
			throw TextException( "Font::renderLatin1MultiLineText : "
				"unable to create temporary word cache since already existing." ) ;

		LogPlug::debug( "Font::renderLatin1MultiLineText : creating a temporary word cache" ) ;

		#endif

		_textCache = new SmartResourceManager<StringColorQualityKey>( DefaultTextCachedQuota, 
			SmartResourceManager<StringColorQualityKey>::DropLessRequestedFirst ) ;

	}

	// Hence in all cases we can rely on having a word cache (even though it starts empty).
   ...
   for ( TextIndex currentLine = 0; currentLine < maxLines; currentLine++ )
   {
       ...
       // Start from the left edge, and select as many words as possible within this line :
       totalWordWidth = 0 ;
       ...
       while ( ! words.empty() && ! lineFull )
       {
    	   ...
    	   {   
    		   wordSurface = & getConstLatin1WordFromCache( currentWord, quality, textColor ) ;
    	...
    	* huge spaces, and the text can stop anywhere before the line's end.
       ...
       if ( justified && ! words.empty() && wordCount > 1 )
       {
    		   ...
    		   it != wordsOnTheLine.end(); it++ ) 
    	   {
    		   ...
    		   {
    			   wordSurface = & getConstLatin1WordFromCache( (*it), quality,
    				   textColor ) ;
    			   wordSurface->blitTo( res, currentWidth, lineHeight ) ;
    		   ...
    		   /*
    			* For justified text, space width is computed with pixel-perfect accuracy.
    			* Knowing exactly what words fit with normal space width, a new space width
    			* is computed so that these words are dispatched regularly on the line, and 
    			* begin and end with it, provided it is not a paragraph end.
    			*
    			* As this space width has to be an integer, round off errors would accumulate
    			* if a constant corrected space width was used, and the last word of the
    			* line would not end perfectly at the end of line, which would lead to a
    			* rather unpleasant visual effect : the right edge of the text would not be
    			* vertically aligned.
    			*
    			* To correct that, after each word the perfect space width for this step is
    			* computed, considering only what remains to be written. Hence the space width
    			* is adapted and the text fit perfectly on the line.
    			* 
    			* Better round to lowest integer (ceil or static_cast) than to nearest,
    			* since if space width is rounded up (floor) the text might be clipped by 
    			* the line edge.
    			*
    			* Number of spaces is equal to number of remaining words minus one, the 
    			* width of the current justified space is the one that would be chosen if 
    			* it was divided equally for all remaining spaces.
    			*
    			*/
    		   wordCount-- ;
    		   ...
    		   currentWidth += wordWidth + /* justified space */ 
    			   static_cast<Width>( Maths::Round( 
    				   ( (Ceylan::Float32) width - currentWidth - totalWordWidth ) 
    					   / wordCount ) ) ;
	...
	if ( createTemporaryWordCache )
	{
		...
		#ifdef OSDL_DEBUG_FONT
		LogPlug::debug( "Font::renderLatin1MultiLineText : deleting temporary word cache : "
			+ _textCache->toString() ) ;
		#endif
	...
	if ( renderIndex == text.size() )
		LogPlug::debug( "Font::renderLatin1MultiLineText : full text fit in box." ) ;
	else	
	...
	throw( TextException )
{
	...
	blitLatin1MultiLineText( targetSurface, clientArea.getUpperLeftAbscissa(),
		clientArea.getUpperLeftOrdinate(), clientArea.getWidth(), clientArea.getHeight(),
		text, renderIndex, quality, textColor, justified ) ;

}


void Font::blitLatin1MultiLineText( Surface & targetSurface, Coordinate x, Coordinate y,
		Length width, Length height, const std::string & text, TextIndex & renderIndex,
		RenderQuality quality, Pixels::ColorDefinition textColor, bool justified ) 
	throw( TextException )
{
	...
	// Nothing to optimize at this level, user ought cache multiline renderings if appropriate.
	Surface * res = & renderLatin1MultiLineText( width, height, text, renderIndex, 
		lastOrdinateUsed, quality, textColor, justified ) ;
...
{ 
	...
	switch( _cacheSettings )
	{
		...
		case GlyphCached:
			res += ". Glyph cache state is : " + _glyphCache->toString( level ) ;
			break ;
...
OSDL::Video::Surface & Font::renderLatin1TextWithWordCached( const string & text, 
	RenderQuality quality, Pixels::ColorDefinition textColor ) throw( TextException )
{
	 ...
	 *
	 * Two passes are therefore needed : one to know the size and create the overall surface, the
	 * other to blit words onto it.
	 *
	 */
	...
	// First iteration : feeds the cache with word renderings and computes the total width :
	for ( list<string>::const_iterator it = words.begin(); it != words.end(); it++ )
	{
		...
		{
			#ifdef OSDL_DEBUG_FONT
			LogPlug::debug( "Font::renderLatin1TextWithWordCached : jumping a space." ) ;
			#endif
		...
		#ifdef OSDL_DEBUG_FONT
		LogPlug::debug( "Font::renderLatin1TextWithWordCached : examining '" + (*it) + "'." ) ;
		#endif
		
		/*
		 * Note that this method is used in the 'word cached' case.
		 * In all cases the returned 'const Surface' is still owned by the cache.
		 *
		 */
		wordRendered = & getConstLatin1WordFromCache( (*it), quality, textColor ) ;
	...
	Surface & res = * new Surface( Surface::Hardware | Surface::ColorkeyBlit, 
		currentWidth, getLineSkip() - getDescent(), 32, redMask, greenMask, blueMask, 
		/* no alpha wanted */ 0 ) ;
	 ...
	 * Surfaces from first iteration could have been stored to save the efforts needed to find 
	 * them in cache, but depending on the cache quota and policy, it cannot be assume their 
	...
	for ( list<string>::const_iterator it = words.begin(); it != words.end(); it++ )
	{
		...
		#ifdef OSDL_DEBUG_FONT
		LogPlug::debug( "Font::renderLatin1TextWithWordCached : blitting '" + (*it) + "'." ) ;
		#endif
...
OSDL::Video::Surface & Font::renderLatin1TextWithTextCached( const string & text, 
	RenderQuality quality, Pixels::ColorDefinition textColor ) throw( TextException )
{
	...
	/*
	 * First check that the text-quality-color combination is not already available 
	 * in cache :
	 *
	 */
...
void Font::blitLatin1Word( Surface & targetSurface, Coordinate x, Coordinate y, 
		const std::string & word, RenderQuality quality, Pixels::ColorDefinition wordColor ) 
	throw( TextException )
{
	 ...
	 *
	 * Hence there are two different cases : either we are word-cached, and we may have a 
	 * rendering in cache, or not.
	 *
	 */
	 
	// Uncomment to search in text cache too (not recommended since not more efficient) : 
	...
	#endif
	{
		...
		getConstLatin1WordFromCache( word, quality, wordColor ).blitTo( targetSurface, x, y ) ;
										
	}
	else
	{
		// Here we cannot cache words, we blit it as directly as possible :
		basicRenderLatin1Text( word, quality, wordColor ).blitTo( targetSurface, x, y ) ;
	}
...
/*
const OSDL::Video::Surface & Font::getConstRenderingForLatin1Word( const std::string & word,
	RenderQuality quality, Pixels::ColorDefinition wordColor ) throw( TextException )
{
	
	// Cannot exist since the caller should not have to deallocate it.
	...
	RenderQuality quality, Pixels::ColorDefinition wordColor ) throw( TextException )
{
	...
	if ( inCache != 0 )
	{
		...
		LogPlug::debug( "Font::getConstLatin1WordFromCache : "
			"cache hit for '" + word + "', returning 'const' prerendered word." ) ;
	...
	#ifdef OSDL_DEBUG_FONT
	LogPlug::debug( "Font::getConstLatin1WordFromCache : "
		"cache miss for '" + word + "', rendering it and submitting it to cache." ) ;
	#endif	
	...
	catch( const ResourceManagerException & e )
	{
		...
		/*
		 * This really should never happen : we know this word rendering is not in cache.
		 *
		 */
	...
	RenderQuality quality, Pixels::ColorDefinition textColor ) throw( TextException )
{
	...
	/*
	 * When adding a letter y after a letter x, y has to be drawn at : abscissa of x plus x's 
	 * advance, so that if x leaves room in the baseline, y can start as left as possible, even if
	 * x spreads on the right of the position where y starts (ex : if x is a capital 'L' whose
	 * bottom line goes under the baseline).
	 * However when a letter is the last in a surface, one should use its width instead of its
	 * advance, so that the letter is not truncated. 
	 * Moreover, the (n-1) letter might spread more to the right than the n one (ex : 'y.', the dot
	 * can be placed at the left of the rightmost branch of the 'y'), so the best solution is to
	 * record max width at each step (instead of replacing the advance by the width for the last
	 * glyph).
	 *
	 */
	Length width = 0 ;
	...
	for ( string::const_iterator it = text.begin(); it != text.end(); it++ )
	{
		...
		{	
			...
			/*
			 * The offset corresponds to the abscissa of the leftmost part of the glyph in its
			 * local referential.
			 *
			 */			
		...
		/*
		 * Previously using next addition, which leads to too much space between letters and not to
		 * compact uppercase letters (ex : 'OSDL') whereas they should be 
		 * (see with cursive fonts with really wide uppercase glyphs, such as 'cretino.ttf').
		 *
		 */
	 ...
	 * (character width should be used instead of advance so that its rightmost part is not 
	 * truncated).
	 *
	 */
 	maxWidth = horizSteps[ textSize - 1 ] + currentOffset + getWidth( currentChar ) ;
	...
	// Must be filled with glyphs now (lineskip is used to avoid complex blits of lines) :
	...
	try
	{
		res = new Surface( Surface::Hardware | Surface::ColorkeyBlit, 
			maxWidth, lineSkip - getDescent(), 32, redMask, greenMask, blueMask, 
			/* no alpha wanted */ 0 ) ;
	}
	catch( const VideoException & e )
	{
		throw TextException( "Font::basicRenderLatin1Text : surface creation failed : "
			+ e.toString() ) ;
	}
	...
	for ( string::const_iterator it = text.begin(); it != text.end(); it++ )
	{
		...
		blitLatin1Glyph( *res, horizSteps[charCount], 
			ascent - getHeightAboveBaseline( currentChar ), currentChar, quality, textColor ) ;
		charCount++ ;
	}
