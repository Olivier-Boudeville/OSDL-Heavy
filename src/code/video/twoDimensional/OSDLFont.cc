  ...
  Pixels::ColorDefinition textColor )
{
   ...
   *
   * Two passes are therefore needed: one to know the size and create the
   * overall surface, the other to blit words onto it.
   *
   */
  ...
  /*
   * First iteration: feeds the cache with word renderings and computes the
   * total width.
   *
   */
		...
		it != words.end(); it++ )
  {
	...
	/*
	 * Note that this method is used in the 'word cached' case. In all cases the
	 * returned 'const Surface' is still owned by the cache.
	 *
	 */
  ...
  RenderQuality quality, Pixels::ColorDefinition textColor )
{
  ...
  for ( string::const_iterator it = text.begin(); it != text.end(); it++ )
  {
	...
	LogPlug::debug( "Font::basicRenderLatin1Text: rendered '"
	  + Ceylan::toString( currentChar )
	  + "' at width "  + Ceylan::toString( horizSteps[charCount] )
	  + ", at height "
	  + Ceylan::toString( ascent - getHeightAboveBaseline( currentChar ) )
