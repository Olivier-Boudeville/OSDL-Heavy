	...
	Pixels::ColorDefinition colorDef, bool filled ) const
{
	...
	/*
	 * Do not forget to substract one from both lengths, since starting from
	 * zero:
	 *
	...
	Pixels::ColorDefinition backgroundColorDef ) const
{
		...
		|| ( backgroundColorDef.unused != Pixels::AlphaOpaque ) )
	{
		 ...
		 *
		 * Using alpha used to lead to nasty rounding errors in colors that make
		 * corners (actually the whole split disc) appear slightly differently
		 * from other parts of the rectangle.
		 *
		 * One way of avoiding having different rounding errors for colors
		 * between the disc and the rectangles would be to have the same pixel
		 * format for the disc than the target surface.
		 *
		 * However the target surface may not have an alpha channel or room for
		 * it (ex: already fully packed 16 bit per pixel), hence we use a full
		 * 32 bit to make any new alpha fit in overall pixel.
		 *
		 * So the ugly artifact is corrected now, at the expense of a rather
		 * complicated code, which may fail with (very) uncommon pixel formats
		 * where there would be no room for a leading or trailing byte which
		 * could be dedicated to the alpha coordinate on a 32 bit pixel.
		 *
		 */
		...
		if ( alphaMask == 0 )
		{
			...
			if ( alphaMask & ( redMask | greenMask | blueMask ) )
			{
				...
				/*
				 * Damned, this one collides with other color masks, try last
				 * chance:
				 *
				 */
			...
			else
			{
				...
				/*
				 * First alpha proposal is ok, let's keep it (3 byte offset to
				 * have alpha):
				 *
				 */
				alphaShift = 24 ;
	...
	else
	{
		 ...
		 *
		 * As the previous method does not work properly in this case (rounding
		 * errors which in this case make corners appear with a slightly
		 * different color from the inner color, again!), we use here a
		 * colorkey:
		 *
		 */
		...
		/*
		 * Picking a color key not corresponding to any of the two disc colors:
		 * rare colors (ex: Pixels::Bisque) are not used as color keys to avoid
		 * bugs very hard to detect.
		 *
		 */
	...
	/*
	 * Blits in place the four corners (clipping might occur if 'target' is too
	 * small):
	 *
	 */
	...
	try
	{
		...
		/*
		 * Both following '1' have be added so that the upper pixel of the disk,
		 * alone in its row (ordinate) does not seem to be a disturbing visual
		 * artefact.
		 *
		 */
	 ...
	 * actually it is a consequence of the disc drawing, which expands quite a
	 * lot of the left.
	 *
	 * Moving the edge one pixel to the right would lead to an offset with the
	 * top and bottom fronteer with the disc.
	 *
	 */
	...
	/*
	 * With very thin edges, one may notice this edge is thinner.  It is the
	 * cost of having circles that can finish on each side of a pixel. It can be
	 * hardly noticed, and prevents artefacts that are a lot more visually
	 * disturbing.
	 *
	 */
	...
	const
{
	 ...
	 *
	 * - necessarily, less than half of the smallest dimensions (otherwise
	 * cannot fit)
	 *
	 * - preferably, not too big (otherwise too much room lost in container)
	 *
	 * - preferably, not too close to edge width, so that there is really an
	 * inner disc
	 *
	 */
