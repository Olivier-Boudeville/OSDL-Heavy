...
const Point3D Point3D::Origin( (Coordinate) 0, 0, 0 ) ;
...
void Point3D::Translate( list<Point3D *> & pointList, Offset x, Offset y, Offset z ) throw()
{
	for ( list<Point3D *>::iterator it = pointList.begin(); it != pointList.end(); it++ )
		(*it)->translate( x, y, z ) ;
}
