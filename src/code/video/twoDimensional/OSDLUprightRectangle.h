...
bool operator == ( const OSDL::Video::TwoDimensional::UprightRectangle & first, 
...
namespace OSDL 
{
	...
	namespace Video 
	{
		...
		namespace TwoDimensional
		{
			...
			// UprightRectangle are defined thanks to 2D points.
			class Point2D ;
		
	 
			
			/**
			 * This classes defines a rectangle whose sides are parallel or orthogonal to
			 * the screen's sides.
			 *
			 * It is the user responsability to provide a set of coordinates so that the upper left
			 * corner is indeed higher than the lower right corner, and at its left, in usual
			 * OSDL 2D referential.
			 *
			 * Failure to do so result in an exception to be raised.
			 *
			 * @note When width or height for an upright rectangle is to read, even for its own
			 * implementation or the one of its children, getWidth() and getHeight() should always
			 * be used instead of _width and _height, since one child class may overload the two
			 * get methods (see Surface for an example).
			 *
			 */
			class UprightRectangle : public Ceylan::TextDisplayable
			{
				...
				public:
					...
					explicit UprightRectangle( const SDL_Rect & source ) throw() ;
					...
					/**
					 * Sets this UprightRectangle's upper left corner, width and height 
					 * do not change.
					 *
					 */
					 ...
					 *
					 * @throw VideoException if width or height would become negative.
					 *
					 */
					 ...
					 * 
					 * @param target the surface to which this rectangle will be rendered.
					 *
					...
					/**
					 * Draws this UprightRectangle to specified surface with rounded corners, 
					 * drawn with specified color, above a background of specified color. The width
					 * of the borders can be chosen, and should not be incompatible with the 
					 * dimensions of this rectangle.
					 * 
					 * @param target the surface to which this rectangle will be rendered.
					 *
					 * @param edgeWidth the width of the edges of this rectangle.
					 *
					 * @param edgeColorDef the color definition of the edges of this rectangle.
					 *
					 * @param backgroundColorDef the color definition of the background of this
					 * rectangle.
					 *
					 * @note Locks surface if needed.
					 *
					 */
					virtual bool drawWithRoundedCorners( Surface & target, Length edgeWidth = 3, 
					...
					/**
					 * Computes the preferred radius of round corners for this rectangle, with the
					 * specified edge width. 
					 *
					 * @param edgeWidth the edge width to take into account for the radius.
					 *
					 * @return a suitable radius, whose dimension is deemed appropriate.
					 *
					 * @throw VideoException if no radius can fullfill the set of constraints due
					 * to this rectangle and the edge width.
					 *
					 */
					 ...
					 *
					 * It is up to the caller to release memory for this SDL_Rect.
					 * Example : SDL_Rect * myRect = aSurface.toSDLRect(); delete myRect ;
					 *
					 */
				...
				private:
					 ...
					 *
					 */			 
					explicit UprightRectangle( const UprightRectangle & source ) throw() ;
