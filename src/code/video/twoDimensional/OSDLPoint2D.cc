...
const Point2D Point2D::Origin( (Coordinate) 0, 0 ) ;
...
Point2D & Point2D::CreateFrom( FloatingPointCoordinate x, FloatingPointCoordinate y ) throw()
{
	return * new Point2D( static_cast<Coordinate>( x ), static_cast<Coordinate>( y ) ) ;
}
...
void Point2D::setTo( FloatingPointCoordinate x, FloatingPointCoordinate y ) throw() 
{
...
void Point2D::Translate( list<Point2D *> & pointList, Offset x, Offset y ) throw()
{
	for ( list<Point2D *>::iterator it = pointList.begin(); it != pointList.end(); it++ )
		(*it)->translate( x, y ) ;
}
