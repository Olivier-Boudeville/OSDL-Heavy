...
TextWidget::TextWidget( 
	...
	BackBufferedWidget( 
			 ...
			 backgroundColor /* will be set later in this constructor for RoundCorners */, 
			 title,
			 minMaximizable,
			 draggable, 
			 wrappable, 
		...
		_currentIndex( 0 )
{
	...
	switch( _shape )
	{
		...
		case RoundCorners:			
			...
			setBaseColorMode( Colorkey, Pixels::selectColorDifferentFrom( _textColor, _edgeColor,
				_backColorForRoundCorners ) ) ;
			break ;
			
		default:
			LogPlug::error( "TextWidget constructor : unexpected shape specified." ) ;
			break ;
			
	}		
	
	
	// Must be called in this constructor as well, so that the overriden version is used :
	updateClientArea() ;
		
	// _needsRedraw set to true thanks to Surface constructor.
	
	// _needsBackbufferRedraw set to true thanks to BackBufferedWidget constructor.
...
void TextWidget::redrawBackBuffer() throw()
{
	...
	renderedText->drawHorizontalLine( 0, renderedText->getWidth(), lastOrdinateUsed, 
		Pixels::Green ) ;					
	*/
	...
	{	
		...
		// Here no color key is to be handled since rectangular shapes take the full room.
		

		// Reduce the height if requested and possible (i.e. if text smaller than client area :
		if ( _minimumHeight && ( lastOrdinateUsed < _clientArea.getHeight() ) )
		{
				
			#ifdef OSDL_DEBUG_WIDGET
			// Should never happen :
			if ( lastOrdinateUsed + 2 * _ClientOffsetHeight + _TitleBarOffsetOrdinate >
					getHeight() )
				Ceylan::emergencyShutdown( "TextWidget::redrawBackBuffer : "
					"specified maximum height exceeded whereas shrinking" ) ;
			#endif
				
			// Recompute total height from newer client height (automatically updates client area) :
			setHeight( lastOrdinateUsed + 2 * _ClientOffsetHeight + _TitleBarOffsetOrdinate ) ;
			
			/*
			 * Resize operation erased all, hence draw the basic widget decorations in
			 * back-buffer :
			 */
	...
	else
	{
		...
		/*
		 * This code is somewhat hazardous insofar we downsize the round rectangle so that it 
		 * contains just the actual surface of rendered text, whereas the client area has been
		 * computed with the older radius.
		 *
		 * However, since here the only change that can take place is the height being decreased,
		 * though it is not stricty proven it should at most lead to a decrease in the radius that
		 * should result in a client area large enough so that the rendered text should fit.
		 * Moreover, the decrease of radius leads to a wider client area !
		 *
		 * It results in having a client area abit wider and taller than before the text was 
		 * rendered, hence the need to aligne it both vertically and horizontally.
		 *
		 */
		...
		if ( _minimumHeight && ( lastOrdinateUsed < _clientArea.getHeight() ) )
		{
			...
			#ifdef OSDL_DEBUG_WIDGET
			LogPlug::trace( "TextWidget::redrawBackBuffer for round rectangle : "
				"lastOrdinateUsed = " + Ceylan::toString( lastOrdinateUsed ) 
				+ ", client area height = " + Ceylan::toString( _clientArea.getHeight() ) ) ;
			#endif
			...
			if ( _clientArea.getWidth() < renderedText->getWidth() )
				Ceylan::emergencyShutdown( "TextWidget::redrawBackBuffer : "
					"resize for round rectangle led to a client area not wide enough : " 
					"client area width = " + Ceylan::toString( _clientArea.getWidth() ) 
					+ ", rendered text width = " 
					+ Ceylan::toString( renderedText->getWidth() ) ) ;
					
			if ( _clientArea.getHeight() < lastOrdinateUsed )
				Ceylan::emergencyShutdown( "TextWidget::redrawBackBuffer : "
					"resize for round rectangle led to a client area not tall enough : "
					"client area height = " + Ceylan::toString( _clientArea.getHeight() ) 
		...
		/*
		 * Color key is to be managed so that the area outside the corners does not hide anything
		 * behind it.
		 *
		 * Hence we fill this surface (the back-buffer) with the already selected color key :
		 *
		 */		
		fill( _baseColor ) ;
		...
		drawWithRoundedCorners( *this, DefaultEdgeWidth, _edgeColor, _backColorForRoundCorners ) ;


		// Force alignment for both dimensions (_verticallyAligned deemed always true here) :
	...
	//renderedText->savePNG( "rendered-text-" + Ceylan::toString( count )	+ ".png" ) ;
	//count++ ;

	LogPlug::debug( "TextWidget::redrawBackBuffer : client area is " + _clientArea.toString() 
	...
	//_overallSurface->drawBox( _clientArea, Pixels::Yellow, /* filled */ false ) ;
...
const string TextWidget::toString( Ceylan::VerbosityLevels level ) const throw()
{
	...
	if ( _minimumHeight )
		textWidgetList.push_back( "Widget height (currently : " + Ceylan::toString( getHeight() )
			+ ") will shrink, if text is short enough, for recorded maximum height, "
			+ Ceylan::toString( _maximumHeight ) ) ; 
	else		 
		textWidgetList.push_back( "Widget height (" + Ceylan::toString( getHeight() ) 
			+ ") will not shrink even if text short enough." ) ;
				
	textWidgetList.push_back( "Text color is " + Pixels::toString( _textColor )  ) ; 

	textWidgetList.push_back( "Edge color is " + Pixels::toString( _edgeColor )  ) ; 

	textWidgetList.push_back( "Text to display is '" + _text + "'." ) ;
	
	textWidgetList.push_back( "Font informations : " + _font->toString( level ) ) ;
	...
	switch( _quality )
	{
		...
		case Text::Font::Solid:
			textWidgetList.push_back( "Text will be rendered in Solid quality" ) ;
			break ;
			
		case Text::Font::Shaded:
			textWidgetList.push_back( "Text will be rendered in Shaded quality" ) ;
			break ;
			
		case Text::Font::Blended:
			textWidgetList.push_back( "Text will be rendered in Blended quality" ) ;
			break ;
			
		default:	
			textWidgetList.push_back( "Text will be rendered in unknown quality (abnormal)" ) ;
			break ;
...
void TextWidget::updateClientArea() throw()
{
	...
	/*
	 * We have to compute what is the biggest upright rectangle that fits into the one with round
	 * corners (the one that is closest to the inner part of the widget, due to edge width).
	 *
	 * Each corner of the rectangular client area should be half-way of its associated arc of 
	 * circle : the corner is defined by the radius  DefaultEdgeWidth
	 *
	...
	Length projectedRadius = static_cast<Length>( radius * Ceylan::Maths::Sqrt_2 / 2 ) ;
