	...
	namespace Video
	{
		...
		namespace TwoDimensional
		{
			...
			namespace Text
			{
				...
				/**
				 * Describes a font index, when multiple fonts are stored in the
				 * same location.
				 *
				 */
				...
				/**
				 * Models an abstract font, including its rendering style (bold,
				 * italic, etc.).
				 *
				 * For each font, a glyph (a character) or a line of text (a
				 * series of glyphs) can be rendered, according to various
				 * encodings (all fonts should support at least the Latin-1
				 * encoding), qualities (some types of fonts allows for several
				 * ones, others will have the selected quality mapped to
				 * available choices) and text color.
				 *
				 * Various cache settings are supported to speed up the
				 * rendering when the same glyph or text is requested more than
				 * once.
				 *
				 * Most of the time, especially if multiline text rendering will
				 * be requested, the 'WordCached' policy should be selected.
				 *
				 ...
				 * existing background that will be hidden only by the pixels
				 ...
				 *
				 * Hence drawing a text is more difficult than just blitting all
				 * glyphs from the same height, and usually applications do not
				 * use direct glyph rendering: most of the time words or texts
				 * are preferably rendered as a whole.
				 *
				 * As a returned surface is encoded with a RLE-color key and/or
				 * can be hardware, it is especially important to lock this
				 * surface before reading and/or writing its pixels, and to
				 * unlock it afterwards.
				 *
				 * This returned surface is to be owned by the caller of the
				 * render method.
				 *
				 * This caller will have to deallocate it when the surface will
				 * not be of use anymore.
				 *
				 ...
				 *
				 * @note Most lengths are given alongside a directed axis, and
				 * therefore they can be negative.
				 *
				 * The signatures detail rendering parameters in the names (ex:
				 * 'renderUTF8Text' instead of simply 'render'), since otherwise
				 * there would be methods whose names would be hidden in child
				 * classes.
				 *
				 * Each child class should initialize itself the '_spaceWidth'
				 * and '_alineaWidth' members, it cannot be done at the level of
				 * this Font mother class.
				 *
				 */
				class OSDL_DLL Font : public Ceylan::TextDisplayable
				{
					...
					public:
						...
						/**
						 * Describes the various rendering qualities available
						 * for texts.
						 *
						 * Note that not all qualities are available for all
						 * fonts.
						 *
						 * This applies mostly for TrueType fonts, since other
						 * systems seldom have different qualities.
						 *
						 * The actual rendering quality will be the closest
						 * available one, compared to the requested one.
						 *
						 * These information on the internal format of the
						 * rendered surfaces are valid only as long as no
						 * conversion to display occurs:
						 *
						 * - Solid: Quick and Dirty
						 *
						 * Creates an 8-bit palettized surface and renders the
						 * given text at fast quality with the given font and
						 * color.
						 *
						 * The 0 pixel value is the colorkey, giving a
						 * transparent background, and the 1 pixel value is set
						 * to the text color.
						 *
						 * The colormap is set to have the desired foreground
						 * color at index 1, this allows you to change the color
						 * without having to render the text again.
						 *
						 * Colormap index 0 is of course not drawn, since it is
						 * the colorkey, and thus transparent, though its actual
						 * color is 255 minus each RGB component of the
						 * foreground.
						 *
						 * This is the fastest rendering speed of all the
						 * rendering modes.
						 *
						 * This results in no box around the text, but the text
						 * is not as smooth as obtained with other qualities:
						 * the 'Solid' one is quite poor.
						 *
						 ...
						 * Blended one.
						 *
						 * Use this mode for frame per second counters and other
						 * fast changing text displays.
						 *
						 * - Shaded: Slow and Nice (and Recommended quality for
						 * most outputs)
						 *
						 * Creates a 32-bit RGB RLE-colorkeyed surface (with no
						 * alpha) and renders the given text at high quality
						 * with the given font and colors.
						 *
						 * Apart the background, the other pixels have varying
						 * degrees of the foreground color from the background
						 * color.
						 *
						 * This background color should be managed as a state
						 * variable of the corresponding font object. Its
						 * default value is pure black.
						 *
						 * The text is antialiased.
						 *
						 * This will render slower than Solid, but in about the
						 * same time as Blended mode.
						 *
						 * The resulting surface should blit quite as fast as
						 * Solid, once it is made.
						 *
						 ...
						 * another image.
						 *
						 * Creates a 32-bit ARGB surface and render the given
						 * text at high quality, using alpha blending to dither
						 * the font with the given color.
						 *
						 * This results in a surface with alpha transparency, so
						 * you do not have a solid colored box around the text.
						 *
						 * The text is antialiased.
						 *
						 * This will render slower than Solid, but in about the
						 * same time as Shaded mode.
						 *
						 * The resulting surface will blit slower than if you
						 * had used Solid or Shaded.
						 *
						 * Use this when you want high quality, and the text is
						 ...
						 * and the key is made from the corresponding text (be
						 * it Latin-1 char or std::string) <em>and</em> from the
						 * text color <em>and</em> from the desired rendering
						 * quality.
						 *
						 ...
						 * color and a quality), which could be modeled at least
						 * thanks to two patterns:
						 *
						 *  - a triplet made from STL: when quality was not
						 * specifically managed, a
						 * std::pair<Latin1Char,ColorDefinition> could be used,
						 * since the STL pairs have the '<' (less) operators
						 * defined (necessary for the ordering of the std::map
						 * ordering).  However ColorDefinition is a struct, with
						 * no specific '<' operator defined, which would be
						 * needed here. Moreover, for three parameters, the STL
						 * is probably less useful
						 *
						...
						/**
						 * Cache key to manage colored characters with various
						 * qualities.
						 *
						 * @note Must be as lightweight as possible, it is a
						 * search key. Virtual table is avoided.
						 *
						 */
						...
						/**
						 * Cache key to manage colored strings, both for word
						 * only and full text.
						 *
						 */
						 ...
						 * they can be put in cache so that the same request is
						 * not rendered twice.
						 *
						 * The various settings for the render cache are:
						 *
						 * - None: when no caching is wanted (prefer memory to
						 * CPU)
						 *
						 * - GlyphCached: put in cache individual glyphs
						 *
						 * - WordCached: put in cache full words. This can be
						 * useful for example with Truetype fonts, since a word
						 * cannot be easily rendered from the set of its glyphs,
						 * due, for instance, to kerning
						 *
						 * - TextCached: allows for full lines, including
						 * whitespaces, to be cached as blocks. Useful when the
						 * same sequence of words has to be rendered multiple
						 * times.
						 *
						 */
						enum RenderCache {
						...
						/**
						 * Describes which of the cache policies made available
						 * by the Smart Resource manager are allowed for fonts.
						 *
						 * @see Ceylan::SmartResourceManager for the description
						 * of the various allowed policies.
						 *
						 * This enum is a convenient way too of overcoming the
						 * impossibility of specifying a cache policy
						 * independently of the key chosen to instantiate the
						 * manager: this is an additional level of indirection,
						 * so that in the font constructor the cache policy can
						 * be specified directly, and not as:
						 ...
						 * expressed according to OpenGL convention nonetheless.
						 *
						 * If the conversion to display is chosen, then at each
						 * change of the display format, the renderings will
						 * have to be converted again.
						 *
						 * @param cacheSettings determines what will be put in
						 * cache in terms of rendering (glyphs, or words, or
						 * texts, etc.)
						 *
						 * @param cachePolicy determines how the cache should
						 * behave regarding renderings being put in cache (store
						 * everything, enforce a maximum size in memory, etc.)
						 *
						 * @param quota Should an upper-bound to the memory size
						 * of cached renderings apply (depending on the chosen
						 * cache policy), determines its actual value.
						 *
						 * If a null (0) quota is passed, then the default value
						 * of the quota for the selected cache settings will be
						 * chosen.
						 *
						 * For example, if cacheSettings is 'WordCached' and
						 * cachePolicy is 'DropLessRequestedFirst' (this policy
						 * uses a quota), and if a null quota is specified, then
						 * the actual quota being used will be
						 ...
						 *
						 * @throw FontException if the specified style is not
						 * supported.
						 *
						 * @note Some fonts (ex: some fixed fonts) support only
						 * the rendering style they had at their creation.  In
						 * this case, this method throws systematically an
						 * exception if the rendering style is changed.
						 *
						 * The solution would be to create a new Font subclass
						 * with the desired rendering style, instead of changing
						 * an already existing font instance.
						 *
						 */
						 ...
						 *
						 * @note The concept of background color only applies
						 * when the 'Shaded' quality is chosen, since with other
						 * rendering qualities the background is transparent.
						 *
						 */
						...
						/**
						 * Returns the width of the specified glyph, rendered
						 * with this font.
						 *
						 * @param character the character whose width will be
						 * returned.
						 *
						 * @note There is no argument-less getWidth since for
						 * most fonts it depends too heavily on the selected
						 * glyph.
						 *
						 * @throw FontException if the glyph metrics could not
						 * be retrieved.
						 *
						 */
						 ...
						 *
						 * This offset corresponds to the first abscissa from
						 * which the glyph should be rendered from a given
						 * location.
						 *
						 * @param character the character whose abscissa offset
						 * will be returned.
						 *
						 * @throw FontException if the glyph metrics could not
						 * be retrieved.
						...
						/**
						 * Returns the height above baseline of the specified
						 * glyph, rendered with this font.
						 *
						 * @param character the character whose height above
						...
						/**
						 * Returns the advance of the specified glyph, rendered
						 * with this font, which is the space between the
						 * leftmost part of the glyph bounding box and the same
						 * part of the next glyph.
						 *
						 * It includes therefore the width of this glyph and the
						 * space between the next glyph that could be rendered
						 * afterwards.
						 *
						 * @param character the character whose advance will be
						 * returned.
						 *
						 * @throw FontException if the glyph metrics could not
						 * be retrieved.
						...
						/**
						 * Returns the width of the recommended space between
						 * two glyphs.
						 *
						 * Simple text rendering can rely on constant space
						 * between glyphs, but for more complex cases such as
						 * multi-line printing, more complex rules should be
						 * used.
						 *
						 * @return a value computed that is an evaluation which
						 * may not be consistent with the character-based
						 * advances that can be requested.
						 *
						 */
						 ...
						 * this font.
						 *
						 * You may use this height for rendering text as c lose
						 * together vertically as possible, though adding at
						 * least one pixel height to it will space it, so they
						 * cannot touch.
						 *
						 * Remember that multiline printing is not enabled, so
						 * the user is responsible for line spacing.
						 *
						 * @see getLineSkip
						 *
						 */
						 ...
						 * relative to a bottom point, by combining it with the
						 * glyph's vertical metrics to resolve the top of the
						...
						/**
						 * Returns the recommended pixel height of a rendered
						 * line of text of this font.
						 *
						 ...
						 * feature.
						 *
						 * @param newAlineaWidth the new value, in pixel, for
						 * the alinea.
						 *
						 */
						...
						/**
						 * Returns a text describing the metrics of the glyph
						 * corresponding to the specified character.
						 *
						 * @param character the character whose glyph is to be
						 * described.
						 *
						 ...
						 *
						 * Depending on the settings, the returned surface can
						 * be a clone of a pre-rendered (cached) character or,
						 * if not available, be put in cache itself so that it
						 * is rendered once.
						 *
						 ...
						 *
						 * @return a newly allocated Surface, whose ownership is
						 * transferred to the caller.
						 *
						 * This surface is encoded with RLE colorkey for maximum
						 * efficiency.
						 *
						 ...
						 * specified color, in a returned surface, in one line
						 * that will not be broken (no multi-line).
						 *
						 * The caller is responsible for deleting the returned
						 * surface.
						 *
						 ...
						 * specified color, on specified location of given
						 * surface, in one line that will not be broken (no
						 * multi-line).
						 *
						...
						/**
						 * Renders specified text, encoded in Latin-1, in
						 * specified color, in a returned surface of specified
						 * dimensions.
						 *
						 * The text will be rendered on as many lines as needed,
						 * and will be automatically justified if requested.
						 *
						 * If the text is too long to fit in the box, then all
						 * the words that fit in will be rendered.
						 *
						 * If a word is too wide to be rendered at all in the
						 * box, then the rendering will stop before it.
						 *
						 * This method will return in 'renderIndex' the index of
						 * the first character from the specified text that
						 * could not have been rendered, if any.
						 *
						 ...
						 * surface.
						 *
						 * @note Multiline text rendering is recommended for use
						 * with one of the two most useful cases regarding text
						 * caches: the 'WordCached' and 'TextCached' policies.
						 *
						 * Using 'GlyphCached' or no cache at all works but is
						 * far less efficient.
						 *
						 * Notably, a temporary word cache will be used, which
						 * may use more peak memory space than wanted.
						 *
						 * @param width the width of the box in which the text
						 * is to be rendered.
						 *
						 * @param height the height of the box in which the text
						 * is to be rendered.
						 *
						 * @param text the text, encoded in Latin-1, to render.
						 *
						 ...
						 * container, then 'renderIndex' will be equal to the
						 * length of 'text'.
						 *
						 * For example, if the text is 'Hello OSDL' and only
						 * 'Hello' could be rendered, then 'renderIndex'
						 * (starting from zero) will be assigned to 6, the index
						 * of the 'O' of OSDL (leading spaces are ignored).
						 *
						 * @param lastOrdinateUsed the method will assign this
						 * user-provided variable the ordinate of the last row
						 * used in the returned surface: depending on the text,
						 * the requested surface will be filled at different
						 * levels.
						 *
						 ...
						 * justified, i.e. should be vertically aligned on both
						 * edges.
						 *
						 * @return a newly allocated Surface, whose ownership is
						 * transferred to the caller.
						 *
						 * @note if there is a word too long to fit (even alone)
						 * on one line, the rendering will not fail but will
						 * stop before that word.
						 *
						 * @throw FontException on error.
						 *
						 */
						 ...
						 *
						 * The text will be rendered on as many lines as needed,
						 * and will be automatically justified.
						 *
						 * If the text is too long to fit in the box, then all
						 * the words that fit in will be rendered.
						 *
						 ...
						 * Words are not broken: only full words are rendered.
						 *
						 * @note Multiline text rendering is for the moment only
						 * implemented for the two most useful cases regarding
						 * text caches: this rendering is available for the
						 * 'WordCached' and 'TextCached' policies.
						 *
						 * It is not implemented for 'GlyphCached', or no cache
						 * at all, yet.
						 *
						 * @param targetSurface the surface the text will be
						 * blitted to.
						...
						/**
						 * The name of the environment variable that may contain
						 * directory names that should contain font files.
						 *
						 */
						 ...
						 * specified as the value of the environment variable
						 * named as specified in FontPathEnvironmentVariable
						 * (FONT_PATH).
						 *
						 */
						...
						/**
						 * Returns a description of the specified rendering
						 * style.
						 *
						...
						/**
						 * Defines the default quota value (maximum size of
						 * cached surfaces in memory, in bytes) if the cache is
						 * glyph-based.
						 *
						...
						/**
						 * Defines the default width for an alinea as a multiple
					...
					protected:
						 ...
						 * quality, in specified color, so that the word cache
						 * is used.
						 *
						 * This method is meant to be used as an helper function
						 * so that user-exposed methods can be easily
						 * implemented.
						 *
						...
						/**
						 * Renders specified Latin-1 text with specified
						 * quality, in specified color, so that the text cache
						 * is used.
						 *
						 * This method is meant to be used as an helper function
						 * so that user-exposed methods can be easily
						 ...
						 *
						 * @param targetSurface the surface the word will be
						 * blitted to.
						 *
						 * @param x the abscissa in target surface of the
						 ...
						 *
						 * @param quality the chosen rendering quality. For
						 * fixed font, only the 'Solid' quality is available.
						 *
						...
						/**
						 * Ensures that specified word rendering is in text
						 * cache, and returns a 'const' reference to it: if the
						 * rendering is not already in text cache, it will be
						 * added.
						 *
						 * This helper method is especially useful for blits,
						 * since otherwise they would have to call a render
						 * method which would create a copy for the cache,
						 * whereas blits can use the cached entry directly. It
						 * saves an useless copy/deleting.
						 *
						 * It does not check the cache for a previous entry
						 * since its purpose is to feed the text cache. It does
						 * that independently of the cache settings (glyph,
						 * word, text, etc.).
						 *
						 * If the 'NeverDrop' cache policy is chosen, then one
						 * should ensure the corresponding cache key is not
						 * already in use.
						 *
						 ...
						 * @param wordColor the color definition for the word.
						 *
						 * @return a 'const' reference to a surface which is a
						 * rendering of the specified word. The surface is still
						 * owned by the cache, and should therefore not be
						...
						/**
						 * Renders specified Latin-1 text with specified
						 * quality, in specified color, directly thanks to the
						 * font backend: no cache is taken into account.
						 *
						 * This method is meant to be used as an helper function
						 * so that user-exposed methods can be easily
						 * implemented.
						...
						/**
						 * Tells whether returned and cached renderings should
						 * be converted to display beforehand.
						 *
						 */
						...
						/**
						 * The smart resource manager that would cache rendered
						 * glyphs, should the GlyphCached render cache be
						 * selected.
						 *
						 * The keys used to specify a glyph are made of a
						 * Latin-1 character and a color specification.
						 *
						 */
						...
						/**
						 * The smart resource manager that would cache rendered
						 * words and/or text, should the WordCached or
						 * TextCached render cache be selected.
						 *
						 * The keys used to specify a glyph are made of a
						...
						/**
						 * Defines what is the background color, for 'Shaded'
						 * quality.
						 *
						 */
						...
						/**
						 * Records the width of an alinea space, in pixels, to
						 * be put at the beginning of a paragraph.
						 *
						 * @note All child classes should initialize it
						 * appropriately, in their constructors.
						 *
