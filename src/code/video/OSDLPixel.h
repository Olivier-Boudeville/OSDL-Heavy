...
namespace OSDL
{
	...
	namespace Video
	{
		 ...
		 *
		 * Various conversion functions are provided, when no canonic operator
		 * can be used.
		 *
		 * For example, comparing two PixelColor instances can be done directly
		 * thanks to the '==' operator, provided they respect the same pixel
		 * format.
		 *
		 */
		namespace Pixels
		{
			...
			/**
			 * Corresponds to a bit mask which allows to define how to get color
			 * coordinates from a PixelColor, for example depending on the
			 ...
			 * 32-bit color space, in this order: the latest coordinate is the
			 * pixel's alpha channel.
			 *
			 * @note The alpha coordinate is not always taken into account and
			 * reliable.
			 *
			...
			/**
			 * Describes a color definition, not converted to a specific format.
			 *
			 */
			 ...
			 * that gamma adjustment is not supported: not all display hardware
			 * support it.
			 *
			 */
			 ...
			 *
			 * Each gamma ramp represents a mapping between the input and output
			 * for that color coordinate.
			 *
			 * The input is the index into the array, and the output is the
			 * 16-bit gamma value at that index, scaled to the output color
			 * precision.
			 *
			 * You may pass a null pointer (O) to any of the channels to leave
			 * them unchanged.
			 *
			 * @note This function adjusts the gamma based on lookup tables, you
			 * can also have the gamma calculated based on a "gamma function",
			 * parametered with setGamma.
			 *
			 * @return true iff the operation is a success. An error could be
			 * that gamma adjustment is not supported, since not all display
			 * hardware support it.
			 *
			 * @fixme Ownership of specified buffers is not clear. Who should
			 * deallocate them? We suppose the caller must allocate them.
			 *
			 */
			 ...
			 *
			 * @fixme Ownership of specified buffers is not clear. Who should
			 * allocate them? We suppose the caller must allocate them.
			 *
			 */
			 ...
			 * the endianness is taken into account, regardless of any specific
			 * pixel format.
			 *
			 * This method just chooses, among the various 32 bit combinations
			 * of masks, one that would be suitable for most uses, including for
			 * OpenGL textures.
			 *
			 * @param format the pixel format from which masks are to be read.
			 *
			 * @param redMask the variable in which this method will put the red
			 * mask.
			 *
			 * @param greenMask the variable in which this method will put the
			 * green mask.
			 *
			 * @param blueMask the variable in which this method will put the
			 * blue mask.
			 *
			 * @param alphaMask the variable in which this method will put the
			 * alpha mask.
			 *
			 * @see getColorMasks
			 *
			...
			/**
			 * Returns the RBG masks which are recommended on this platform: the
			 * endianness is taken into account, regardless of any specific
			 * pixel format.
			 *
			 * This method just chooses, among the various 32 bit combinations
			 * of masks, one that would be suitable for most uses, including for
			...
			/**
			 * Reads from specified pixel format the RBGA masks, and returns
			 * them by updating specified color mask references.
			 *
			 * @param format the pixel format from which masks are to be read.
			 *
			 * @param redMask the variable in which this method will put the red
			...
			/**
			 * Converts a set of four coordinates in RGBA color space to the
			 * equivalent ColorDefinition.
			 *
			 * @note If alpha coordinate is not specified (just plain RGB), the
			 * pixel will be considered as fully opaque.
			 *
			 * @note No mapping to any pixel format is performed, it is just two
			 * different ways of gathering the exact same data about color.
			 *
			...
			/**
			 * Returns RGBA quadruplet corresponding to specified pixel, encoded
			 * according to specified pixel format.
			 *
			 * @note If the surface has no alpha component, the alpha will be
			 * returned as AlphaOpaque (100% opaque).
			 *
			...
			/**
			 * Converts a set of four coordinates in RGBA color space, expressed
			 * as a color definition, to a PixelColor, using specified surface
			 * format.
			 *
			 * If the specified pixel format has no alpha component, or if a
			 * palette is used, then the alpha value will be ignored since it
			 * can be in no way be taken into account.
			 *
			 */
			...
			/**
			 * Converts a color definition to a raw PixelColor, regardless of
			 * any pixel format.
			 *
			 * @note No pixel format mapping of any kind is performed, it is a
			 * transformation between two exactly similar formats, as if the
			 * pixel format was exactly 32-bit RGBA.
			 *
			 */
			...
			/**
			 * Converts a RGBA raw color into a raw PixelColor, with no pixel
			 * format mapping.
			 *
			 * @note No pixel format mapping of any kind is performed, it is a
			 * transformation between two exactly similar formats, as if the
			...
			/**
			 * Returns whether the two color definitions describe the exact same
			 * color.
			 *
			 ...
			 *
			 * @param useAlpha tells whether the alpha coordinate is tested too
			 * for equality.
			 *
			 */
			 ...
			 *
			 * @note This function is useful to provide a '<' operator for some
			 * containers (see Text::Font class).
			 *
			 */
			...
			/**
			 * Returns whether the two pixel colors describe the exact same
			 * color, supposing that the two pixel colors are defined according
			 * to the same pixel format.
			 *
			 * @param first the first pixel color for the comparison.
			 *
			 * @param second the second pixel definition for the comparison.
			 *
			 * @note The alpha coordinate cannot be disabled or enabled, this
			 * function just compares the two values and could be directly
			 ...
			 *
			 * It is useful for example when having to find predictably a color
			 * key which does not collide with two used colors.
			 *
			...
			/**
			 * Returns a color definition which is different from the specified
			 * ones.
			 *
			 * The alpha coordinate is not taken into account in comparisons.
			 *
			 * It is useful for example when having to find predictably a color
			 * key which does not collide with a set of used colors.
			 *
			 * @param first the first color to avoid.
			 *
			 * @param second the second color to avoid.
			 ...
			 * format).
			 *
			 * @note No clipping is performed, and the surface should have been
			 ...
			 * format).
			 *
			 * @param fromSurface the surface to read the pixel from. If this
			 * surface has no alpha coordinate, then the returned color
			 * definition will have its fourth color coordinate equal to
			 * AlphaOpaque.
			 *
			 * @note No clipping is performed, and the surface should have been
			 * previously locked if necessary.
			...
			/**
			 * Puts specified pixel at [x;y] with the given color, specified as
			 ...
			 * pixel. If false, the exact specified color will be put in target
			 * pixel, instead of being blended with it.
			 *
			 * @param clipping tells whether point location is checked against
			 * surface bounds. If clipping is activated and the pixel is
			 * outside, nothing is done.
			 *
			 * @param locking tells whether this primitive should take care of
			 * locking / unlocking the surface (not recommended on a per pixel
			 * basis, because of lock overhead).
			 *
			...
			/**
			 * Puts the specified pixel at [x;y] with the given color, specified
			 * as a RGBA color definition, not as a pixel color already encoded
			 * according to the pixel format of target surface.
			 *
			 * If the alpha coordinate is not AlphaOpaque, then the specified
			 * pixel will be alphablended with the pixel already present at
			 * specified location.
			 ...
			 * These color coordinates will be mapped according to the target
			 ...
			 * 32-bit RGBA pixel color, already encoded according to Surface's
			 * pixel format.
			 *
			 * If the alpha coordinate is not AlphaOpaque, then the specified
			 * pixel will be alphablended with the pixel already present at
			 ...
			 * @param y the ordinate of the point to change.
			 *
			 * @param convertedColor the color of the pixel to be put, a 32-bit
			 * pixel value already mapped according to the target surface's
			 * pixel format.
			 *
			 * @param alpha the full alpha coordinate to be used. The one
			 * guessed from the 'convertedColor' parameter would not be enough,
			 * since it would be already encoded and rounded.
			 *
			 * @param blending tells whether the alpha channel must be taken
			 * into account, resulting to alpha blending with the destination
			 * pixel. If false, the exact specified color will be put in target
			...
			/**
			 * Alternative method to set the pixel at [x;y] to the given pixel
			 * color.
			 *
			 * If the alpha coordinate is not AlphaOpaque, then the specified
			 * pixel will be alphablended with the pixel already present at
			 * specified location.
			 *
			 * @param mapToSurfaceFormat tells whether the specified pixel color
			 * is to be encoded to target Surface's pixel format (if true), or
			 * if it is not wanted (if false, for example if it is already
			 * done).
			 *
			 * @note This function should not be used directly, it is merely a
			 * debug method when pixel-level operations must be checked.
			 *
			 */
			 ...
			 * definition.
			 *
			 * @note The alpha coordinate has not always a meaning, depending of
			 * the context.
			 *
			 */
			 ...
			 *
			 * This color is useful to specify that, for example, a background
			 * should remain empty, i.e. fully transparent.
			 *
			 */
