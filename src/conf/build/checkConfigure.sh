...
echo
echo "  + recreating 'configure.ac'"
make -f MakeConfigure clean
make -f MakeConfigure

SOURCE_ROOT="../.."
DEBUG "SOURCE_ROOT = $SOURCE_ROOT"
cd $SOURCE_ROOT

CURRENT_CONFIGURE_FILE=$RUNNINGDIR/$SOURCE_ROOT/configure.ac
...
if [ $RES -eq 0 ] ; then
	...
	if diff $CURRENT_CONFIGURE_FILE $CONF_UPDATED 1>/dev/null 2>&1; then
		...
		$DIFF_TOOL $SOURCE_ROOT/configure.ac $CONF_UPDATED
	fi
else
