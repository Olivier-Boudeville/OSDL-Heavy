...
loani_repository=`pwd|sed 's|/osdl/OSDL/trunk/tools||1'`
...
# Where these tools should be installed:
tool_install_location="$OSDL_PREFIX"

if [ -n "$tool_install_location" ] ; then
	tool_install_location_opt="--prefix=$tool_install_location"
else
	tool_install_location_opt=""
fi


# To check the user can override them:
#tool_overriden_options="CPPFLAGS=\"-DTOOL_CPPFLAGS\" LDFLAGS=\"-LTOOL_LDFLAGS\""
tool_overriden_options=""

configure_opt="-enable-strict-ansi --enable-debug $prerequesites_prefix_opt $tool_install_location_opt $tool_overriden_options"
...
echo
echo "Bootstrapping now build for tool system thanks to the autotools"
...
# Old-fashioned way of regenerating the build system from scratch:
{
	...
	if [ $do_install -eq 0 ] ; then
		...
		if [ -n "$osdl_install_prefix_opt" ] ; then
			echo 1>&2
			echo "Warning: not installing tools and using $osdl_install_prefix_opt implies updating library search paths to select the correct library, for example one may enter: " 1>&2
